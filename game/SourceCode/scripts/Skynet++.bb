...
Global SE_TRANSIENT_STACK_SIZE=512
	...
	If SE_ARGUMENTS_NUMBER
		...
		if Args <> -1 Then
			...
			Elseif Args = 1 Then
				...
				;DebugLog "Add "+SE_ARGUMENTS_STACK(SE_ARGUMENTS_STACK_OFFSET)\IntValue+" type = "+SE_ARGUMENTS_STACK(SE_ARGUMENTS_STACK_OFFSET+1)\IntValue
			Endif
		EndIf
	EndIf
...
;					EndIf
			...
			Case SE_INVG
				...
				SE_InvokeGlobalFunction(ValueA\StaticIndex, FuncPtr)
				SE_RETURN_VALUE=CurrentReturnValue
				SE_ARGUMENTS_NUMBER=CurrentArgsNumber
...
Function SE_InvokeGlobalFunction(FunctionID%, CurrentFuncPtr.SE_FuncPtr)
	SE_ExecuteFunction(FunctionID, CurrentFuncPtr)
End Function	
...
Function SE_IsValidScript(ScriptName$)
	Return (Lower(Right(ScriptName, 4))=".gsc" Or Lower(Right(ScriptName, 5))=".gscc")
End Function
...
;Include compiler
Include "SourceCode\scripts\Skynet++ Compiler.bb"

;Include basic global functions library (contains mathematic, string and array operations)
Include "SourceCode\scripts\Skynet++ Base Lib.bb"
; functions system
Include "SourceCode\scripts\Skynet Functions.bb"
