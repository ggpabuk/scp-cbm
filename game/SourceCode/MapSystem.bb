...
Function ChangeRMesh(r.Rooms, newpath$)
	Local Children[200]

	For i = 1 To CountChildren(r\obj)
		Children[i] = GetChild(r\obj, i)
		EntityParent(Children[i], 0)
	Next
	
	r\RoomTemplate\obj = LoadRMesh(newpath, r\RoomTemplate)
	
	Local x# = EntityX(r\obj)
	Local y# = EntityY(r\obj)
	Local z# = EntityZ(r\obj)
	Local yaw# = EntityYaw(r\obj)
	
	HideEntity(r\obj)
	EntityType(r\obj, 0)
	EntityPickMode(r\obj, 0)
	ResetEntity(r\obj)
	
	Local PrevRoomObject% = r\obj
	
	r\obj = CopyEntity(r\RoomTemplate\obj)
	ScaleEntity(r\obj, RoomScale, RoomScale, RoomScale)
	EntityType(r\obj, HIT_MAP)
	EntityPickMode(r\obj, 2)
	PositionEntity(r\obj, x, y, z)
	RotateEntity(r\obj, 0, yaw, 0)

	For i = 1 To 199
		If Children[i] <> 0 Then EntityParent(Children[i], r\obj)
	Next
...
Function UpdateForest(fr.Forest,ent%, isAlpha% = False)
	;Catch("Uncaught (UpdateForest)")
	;local variables
	Local tx%,ty%, inforest% = False
				...
				If fr\TileEntities[tx+(ty*gridsize)]<>0 Then
					If Abs(EntityX(ent,True)-EntityX(fr\TileEntities[tx+(ty*gridsize)],True))<20.0 Then
						If Abs(EntityZ(ent,True)-EntityZ(fr\TileEntities[tx+(ty*gridsize)],True))<20.0 Then
							...
							inforest = True
							;if isAlpha Then
							;	;EntityAlpha fr.Forest\Forest_Pivot, 0.0
							;	EntityAlpha fr\TileEntities[tx+(ty*gridsize)], 0.0
							;Else
							;	;EntityAlpha fr.Forest\Forest_Pivot, 1.0
							;	EntityAlpha fr\TileEntities[tx+(ty*gridsize)], 1.0
							;EndIf
						Else
...
Function LoadRoomTemplates(file$)
	...
	While Not Eof(f)
		...
		If Left(TemporaryString,1) = "[" Then
			...
			rt\NightObjPath = GetINIString(file, TemporaryString, "night mesh path")
...
Function LoadRoomMesh(rt.RoomTemplates, objpath$="")
	
	If objpath = "" Then objpath = rt\objPath
	
	If Instr(objpath,".rmesh")<>0 Then ;file is roommesh
		rt\obj = LoadRMesh(objpath, rt)
	Else ;file is b3d
		If objpath <> "" Then rt\obj = LoadWorld(objpath, rt) Else rt\obj = CreatePivot()
	EndIf
...
Const MAX_ROOM_DOORS = 20
Const MAX_ROOM_EMITTERS = 20
...
Type Rooms
	...
	Field RoomDoorsOpt.Doors[MAX_ROOM_DOORS]
	Field RoomDoorsCount%
	Field NPC.NPCs[12]
	Field grid.Grids
	...
	Field EventID
	Field IsRoomShowed%, ObjectHided, LightsHided%, RenderLightsHided%, LightsUpdated%, HidedDoors
	Field Prop[128], PropCount
	Field Emitter.Emitters[MAX_ROOM_EMITTERS]
	Field RoomYOffset#, IsRoomDeeped%, MaxRoomDistance#
End Type
...
Type RoomsDoorsLine
	Field Door.Doors
End Type
Global AllowRoomsDoorsInit% = False
...
Function FindFreeRoomID()
	...
	Next
End Function
Function CheckRoomDeep(r.Rooms, IncludeDistance=True)
	If r\IsRoomDeeped Then
		Local dist# = Distance(EntityX(Collider), EntityZ(Collider), EntityX(r\obj), EntityZ(r\obj))
		Return ((((EntityY(Collider)-EntityY(r\obj)+r\RoomYOffset) >= 0.0) And (dist < r\MaxRoomDistance Or (Not IncludeDistance))) Or dist > r\MaxRoomDistance*2)
	Else
		Return (sqrvalue(EntityY(Collider) - EntityY(r\obj)) <= 1.0)
	EndIf
End Function
...
Function CreateRoom.Rooms(zone%, roomshape%, x#, y#, z#, name$ = "")
	...
	Select Lower(name)
		Case "room079"
			r\RoomYOffset = 0.302
			r\IsRoomDeeped = True
			r\MaxRoomDistance = 6.0
		Case "room106"
			r\RoomYOffset = 0.0
			r\IsRoomDeeped = True
			r\MaxRoomDistance = 7.0
	End Select
...
Function FillRoom(r.Rooms)
	...
	Select r\RoomTemplate\Name
		Case "room860"
			...
			AddEntityToRoomProps(r, r\Objects[2])
			AddEntityToRoomProps(r, r\Objects[3])
			AddEntityToRoomProps(r, r\Objects[4])
;			;DrawPortal stuff
...
;			CameraFogColor dp\cam,0,0,0 ;98,133,162
;			CameraFogMode dp\cam,1
			
			;doors to observation booth
			d = CreateDoor(r\zone, r\x + 928.0 * RoomScale,0,r\z + 640.0 * RoomScale,0,r,NetworkServer\Breach,False,False,"ABCD")
			d\AutoClose = Not NetworkServer\Breach
...
;			EntityParent dp\cam,fr\Forest_Pivot
		...
		Case "lockroom"
			...
			d\timer = 70 * (5+(5*NetworkServer\Breach))
			d\AutoClose = False
			d\open = False
			...
			d2\timer = 70 * (5+(5*NetworkServer\Breach))
			d2\AutoClose = False
			d2\open = False
		...
		Case "gatea"
			...
			AddEntityToRoomProps(r, r\Objects[13])
			...
			AddEntityToRoomProps(r, r\Objects[14])
		...
		Case "exit1"
			...
			;3969.68693 9974.92682 5827.87935
		...
		Case "roompj"
			...
			If NetworkServer\Breach = False Then
				...
				EntityParent(it\collider, r\obj)
			Else
				it = CreateItem("Strange Bottle", "veryfinefirstaid", r\x + 800.0 * RoomScale, r\y + 176.0 * RoomScale, r\z + 1108.0 * RoomScale)
				RotateEntity it\collider, 0, r\angle, 0
				EntityParent(it\collider, r\obj)
			EndIf
		...
		Case "room079"
			...
			AddEntityToRoomProps(r, r\Objects[0])
		...
		Case "checkpoint1"
			;[Block]
			r\RoomDoors[0] = CreateDoor(0, r\x + 48.0*RoomScale, 0, r\z - 128.0 * RoomScale, 0, r, False, False, 3-NetworkServer\Breach)
			...
			r\RoomDoors[1] = CreateDoor(0, r\x - 352.0*RoomScale, 0, r\z - 128.0 * RoomScale, 0, r, False, False, 3-NetworkServer\Breach)
		...
		Case "checkpoint2"
			;[Block]
			r\RoomDoors[0]= CreateDoor(0, r\x - 48.0*RoomScale, 0, r\z + 128.0 * RoomScale, 0, r, False, False, 5-(NetworkServer\Breach*2))
			...
			r\RoomDoors[1] = CreateDoor(0, r\x + 352.0*RoomScale, 0, r\z + 128.0 * RoomScale, 0, r, False, False, 5-(NetworkServer\Breach*2))
			;FreeEntity r\RoomDoors[1]\buttons[0]
			;FreeEntity r\RoomDoors[1]\buttons[1]
		...
		Case "room2storage"
			...
			Else
				...
				EntityParent(it\collider, r\obj)
			EndIf
			it = CreateItem("Level "+(NetworkServer\Breach+1)+" Key Card", "key"+(NetworkServer\Breach+1), r\x - 672.0 * RoomScale, r\y + 240.0 * RoomScale, r\z + 224.0 * RoomScale)
			EntityParent(it\collider, r\obj)
			;[End Block]
		Case "room2sroom"
			;[Block]
		...
		Case "room2nuke"
			...
			For n% = 0 To 1
				...
				AddEntityToRoomProps(r, r\Objects[n * 2])
				AddEntityToRoomProps(r, r\Objects[n * 2 + 1])
...
;			
		...
		Case "008"
			...
			EntityParent(r\Objects[1], r\obj)
			AddEntityToRoomProps(r, r\Objects[1])
		...
		Case "room035"
			...
			For i = 0 To 1
				...
				AddEntityToRoomProps(r, r\Objects[i*2])
				AddEntityToRoomProps(r, r\Objects[i*2+1])				
			Next
		...
		Case "room513"
			...
			If Rand(0, 1) And NetworkServer\Breach Then
		...
		Case "room049"
			...
			For n% = 0 To 1
				...
				AddEntityToRoomProps(r, r\Objects[n * 2 + 6])
				AddEntityToRoomProps(r, r\Objects[n * 2 + 7])
			Next
...
;			PositionEntity(r\Objects[6], r\x - 848*RoomScale, 0.5, r\z-576*RoomScale, True)
		...
		Case "room2closets"
			...
			If Not NetworkServer\Breach Then
				...
				EntityParent(it\collider, r\obj)
			Else
				it = CreateItem("Strange Bottle", "veryfinefirstaid", r\x + 736.0 * RoomScale, r\y + 176.0 * RoomScale, r\z - 448.0 * RoomScale)
				EntityParent(it\collider, r\obj)

				it = CreateItem("Strange Bottle", "veryfinefirstaid", r\x + 730.0 * RoomScale, r\y + 176.0 * RoomScale, r\z - 496.0 * RoomScale)
				EntityParent(it\collider, r\obj)
			EndIf
			it = CreateItem("Level 1 Key Card", "key1", r\x + 736.0 * RoomScale, r\y + 240.0 * RoomScale, r\z + 752.0 * RoomScale)
			EntityParent(it\collider, r\obj)
			
			it = CreateItem("Clipboard","clipboard",r\x + 736.0 * RoomScale, r\y + 224.0 * RoomScale, r\z -480.0 * RoomScale)
			EntityParent(it\collider, r\obj)
		...
		Case "room2tesla","room2tesla_lcz","room2tesla_hcz"
			...
			;AddEntityToRoomProps(r, r\Objects[7])
			;[End Block]
		Case "room2doors"
			;[Block]
		...
		Case "173"
			...
			AddEntityToRoomProps(r, r\Objects[9])
			
			r\Objects[10] = LoadMesh_Strict("GFX\map\intro_labels.b3d",r\obj)
			AddEntityToRoomProps(r, r\Objects[10])
			FreeTexture tex
			;[End Block]
		Case "room2ccont"
			;[Block]
		...
		Case "room106"
			...
			EntityParent(r\Objects[6], r\obj)
			AddEntityToRoomProps(r, r\Objects[6])
			...
			For n = 0 To 2 Step 2
				...
				;makecollbox(r\Objects[n * 2 + 1])
				AddEntityToRoomProps(r, r\Objects[n])
				AddEntityToRoomProps(r, r\Objects[n+1])
			Next
								...
								Case (chance<40)
									...
									Select temp3
										Case 1
											...
											tempstr2 = "mp5sd"
										Case 2
											tempstr="Strange Bottle"
											tempstr2 = "veryfinefirstaid"
										Case 3
											tempstr="Strange Bottle"
											tempstr2 = "veryfinefirstaid"
										Case 4
		...
		Case "room1123"
			...
			EntityParent r\Objects[8],r\obj
			AddEntityToRoomProps(r, r\Objects[8])
			...
			EntityParent r\Objects[11],r\obj
			AddEntityToRoomProps(r, r\Objects[11])
			...
			AddEntityToRoomProps(r, r\Objects[12])
		...
		Case "pocketdimension"
			...
			For i = 1 To 8
				...
				If i < 6 Then 
					...
					EntityBlend de\obj, 2
				EndIf
				AddEntityToRoomProps(r, r\Objects[i-1])
			Next
		...
		Case "room1162"
			;[Block]
			d = CreateDoor(r\zone, r\x + 248.0*RoomScale, 0.0, r\z - 736.0*RoomScale, 90, r, False, False, 2-NetworkServer\Breach)
			r\Objects[0] = CreatePivot()
		...
		Case "room2sl"
			...
			For i = 0 To 14
				If i <> 7 Then
					...
					EndIf
					AddEntityToRoomProps(r, r\Objects[i])
				EndIf
			Next
			For i = 0 To 2
			...
			AddEntityToRoomProps(r, r\Objects[9 * 2])
			AddEntityToRoomProps(r, r\Objects[9 * 2 + 1])
...
;			RotateEntity r\Objects[21],8.6,-150.0,-5.0
		...
		Case "lockroom3"
			;[Block]
			d = CreateDoor(r\zone, r\x - 736.0 * RoomScale, 0, r\z - 104.0 * RoomScale, 0, r, True)
			d\timer = 70 * (5+(5*NetworkServer\Breach)) : d\AutoClose = False : d\open = False : d\locked = True
			...
			d2 = CreateDoor(r\zone, r\x + 104.0 * RoomScale, 0, r\z + 736.0 * RoomScale, 270, r, True)
			d2\timer = 70 * (5+(5*NetworkServer\Breach)) : d2\AutoClose = False: d2\open = False : d2\locked = True
			EntityParent(d2\buttons[0], 0)
			PositionEntity(d2\buttons[0], r\x + 640.0 * RoomScale, 0.7, r\z + 288.0 * RoomScale)
			RotateEntity (d2\buttons[0], 0, 90, 0)
			EntityParent(d2\buttons[0], r\obj)
	...
	InitRoomDoors(r)
	AllowRoomsDoorsInit = False
	;Catch("FillRoom ("+r\RoomTemplate\Name+")")
End Function

Function InitAdjacentRoomDoors(r.Rooms)
	Local DoorsCount% = r\RoomDoorsCount
	For i = 0 To 3
		If r\AdjDoor[i] <> Null Then
			For a = 1 To MAX_ROOM_DOORS-1
				If r\RoomDoorsOpt[a] = Null Then
					DoorsCount = a
					r\RoomDoorsOpt[a] = r\AdjDoor[i]
					Exit
				EndIf
			Next
		EndIf
	Next
	r\RoomDoorsCount = DoorsCount
End Function

Function AddDoorToOptSystem(r.Rooms, d.Doors)
	For i = 1 To MAX_ROOM_DOORS-1
		If r\RoomDoorsOpt[i] = Null Then
			r\RoomDoorsCount = i
			r\RoomDoorsOpt[i] = d
			Exit
		EndIf
	Next
End Function
Function InitRoomDoors(r.Rooms, DoorsCount = 0)
	For RoomDoorLine.RoomsDoorsLine = Each RoomsDoorsLine
		If RoomDoorLine\Door <> Null Then AddDoorToOptSystem(r, RoomDoorLine\Door)
	Next
			;founddoor = False
			;For j = 0 To 7
			;	if r\RoomDoors[j] = RoomDoorLine\Door Then
			;		founddoor = True
			;		Exit
			;	EndIf
			;Next
	...
	;		foundDoor = False
	;		For j = 1 To MAX_ROOM_DOORS-1
	;			if r\RoomDoorsOpt[j] = r\RoomDoors[i] Then
	;				foundDoor = true
	;				Exit
	;			EndIf
	;		Next
	;		if not founddoor then
	;			For j = 1 To MAX_ROOM_DOORS-1
	;				if r\RoomDoorsOpt[j] = Null Then
	;					r\RoomDoorsCount = j
	;					r\RoomDoorsOpt[j] = r\RoomDoors[i]
	;					Exit
	;				EndIf
	;			Next
	;		endif
...
Function UpdateRooms()
	...
	;	- ENDSHN
	If (EntityZ(Collider)/8.0)<I_Zone\Transition[1]-(((SelectedMap="" And NetworkServer\ServerCustomMap = "") And NetworkServer\ServerCustomMap = "")) Then
		PlayerZone=2
	ElseIf (EntityZ(Collider)/8.0)>=I_Zone\Transition[1]-((SelectedMap="" And NetworkServer\ServerCustomMap = "")) And (EntityZ(Collider)/8.0)<I_Zone\Transition[0]-((SelectedMap="" And NetworkServer\ServerCustomMap = "")) Then
		PlayerZone=1
	Else
		PlayerZone=0
	EndIf
	...
	If PlayerRoom<>Null Then
		...
		If CheckRoomDeep(PlayerRoom) Then
	...
	;				if EntityDistance(r\obj, p\Pivot) < 5 Then p\PlayerRoomID = r\ID : Exit
	;			Next
	;		EndIf
	;	Next
	;EndIf
	...
	For r.Rooms = Each Rooms
			...
			If (Not foundNewPlayerRoom) And (PlayerRoom<>r) Then				
				If x < 4.0 Then
					If z < 4.0 Then
						...
						If CheckRoomDeep(r,False) Then PlayerRoom = r
						foundNewPlayerRoom = True
					EndIf
				EndIf
			EndIf
		...
		If hide Then
			HideRoomWithCollision(r)
			HideRoomLights(r)
			HideRoomDoors(r, Null)
			HideEntity r\obj
		Else
			ShowRoomWithCollision(r)
			ShowRoomDoors(r)
		EndIf
	...
	If PlayerRoom<>Null Then
		...
		For i=0 To 3
			If PlayerRoom\Adjacent[i]<>Null Then
				ShowRoomWithCollision(PlayerRoom\Adjacent[i])
				ShowRoomDoors(PlayerRoom\Adjacent[i])
				UpdateRoomLights(PlayerRoom\Adjacent[i])
				
				If PlayerRoom\AdjDoor[i]<>Null
					If PlayerRoom\AdjDoor[i]\openstate = 0 Then
						HideRoomDoors(PlayerRoom\Adjacent[i], PlayerRoom\AdjDoor[i])
						HideRoomWithCollision(PlayerRoom\Adjacent[i])
					EndIf
				EndIf
				
				If NetworkServer\LongCulling Then
					Local AccessToShow% = True
					For j = 0 To 3
						If PlayerRoom\Adjacent[i]\Adjacent[j] <> Null Then
							If PlayerRoom\Adjacent[i]\Adjacent[j] <> PlayerRoom Then
								If PlayerRoom\Adjacent[i]\AdjDoor[j]\openstate = 0 Then AccessToShow = False
								
								If (Not AccessToShow) Then
									ShowRoomWithCollision(PlayerRoom\Adjacent[i]\Adjacent[j])
									ShowRoomDoors(PlayerRoom\Adjacent[i]\Adjacent[j])
									UpdateRoomLights(PlayerRoom\Adjacent[i]\Adjacent[j])
								EndIf
							EndIf
						EndIf
					Next
				Else
					For j = 0 To 3
						If PlayerRoom\Adjacent[i]\Adjacent[j] <> Null Then
							If PlayerRoom\Adjacent[i]\Adjacent[j] <> PlayerRoom Then
								ShowEntity(PlayerRoom\Adjacent[i]\Adjacent[j]\obj)
								HideRoomWithCollision(PlayerRoom\Adjacent[i]\Adjacent[j])
								UpdateRoomLights(PlayerRoom\Adjacent[i]\Adjacent[j])
							EndIf
						EndIf
					Next
...
Function HideRoomWithCollision(r.Rooms)
	If r\IsRoomShowed Then
		EntityAlpha(GetChild(r\obj,2),0.0)
		For i = 0 To r\PropCount
			EntityAlpha(r\Prop[i],0.0)
		Next
	...
	EndIf
End Function
Function ShowRoomWithCollision(r.Rooms)
	If Not r\IsRoomShowed Then
		ShowEntity r\obj
		EntityAlpha(GetChild(r\obj,2),1.0)
		For i = 0 To r\PropCount
			EntityAlpha(r\Prop[i],1.0)
		Next
...
Function RenderRoomLights()
	For r.Rooms = Each Rooms
		If Not r\RenderLightsHided Then
			For i = 0 To r\MaxLights%
				HideEntity r\Lights[i]
				HideEntity r\LightSpritesPivot[i]
				HideEntity r\LightSprites2[i]
				HideEntity r\LightSprites[i]
			Next
			r\RenderLightsHided = True
		EndIf
	Next
	Return
	For r.Rooms = Each Rooms
		If r\dist < HideDistance*2 Or r = PlayerRoom Then
			If r\RenderLightsHided = True Then
				For i = 0 To r\MaxLights%
					ShowEntity r\Lights[i]
				Next
				r\RenderLightsHided = False
			EndIf
		Else
			If Not r\RenderLightsHided Then
				For i = 0 To r\MaxLights%
...
Function UpdateRoomLights(r.Rooms)
	If Not r\LightsUpdated Then
		Local i, random#, alpha#, dist#
		r\LightsUpdated = True
		If EnableRoomLights And (SecondaryLightOn>0.5) Then
			For i = 0 To r\MaxLights%
				r\RenderLightsHided = False
				...
				If UpdateRoomLightsTimer=0.0 Then
					...
					If PlayerRoom\RoomTemplate\Name$ = "173" Then
						random# = Rnd(0.38*0.4,0.42*0.4)
					Else
						If r\LightFlicker%[i]<5 Then
							random# = Rnd(0.38*0.4,0.42*0.4)
						ElseIf r\LightFlicker%[i]>4 And r\LightFlicker%[i]<10 Then
							random# = Rnd(0.35*0.4,0.45*0.4)
						Else
							random# = Rnd(0.3*0.4,0.5*0.4)
						EndIf
					EndIf
					...
					;	If EntityDistance(Camera,r\LightSprites2[i])>=8.5 Or (Not EntityVisible(Camera,r\LightSpritesPivot[i])) Then
					;		HideEntity r\LightSprites2%[i]
					;		r\LightSpriteHidden%[i] = True
					;	EndIf
					;EndIf
					...
					If r\LightCone[i]<>0 Then
						...
						If r\LightFlicker%[i]>4 Then
							If Rand(400)=1 Then
								...
								PlaySound2(IntroSFX(Rand(10,12)),Camera,r\LightSpritesPivot[i])
								ShowEntity r\LightConeSpark[i]
					...
					;if Not EntityVisible(Camera, r\LightSpritesPivot[i]) Then 
					;	HideEntity(r\Lights[i])
					;Else
					;	ShowEntity(r\Lights[i])
					;EndIf
	...
	EndIf
End Function
Function HideRoomDoors(r.Rooms, AdjDoorEx.Doors, HideFrame% = True)
	If Not r\HidedDoors Then
		For i = 1 To r\RoomDoorsCount
			If r\RoomDoorsOpt[i]<>Null Then
				If r\RoomDoorsOpt[i] <> AdjDoorEx Then
					If r\RoomDoorsOpt[i]\obj <> 0 Then EntityAlpha r\RoomDoorsOpt[i]\obj, 0.0
					If r\RoomDoorsOpt[i]\frameobj <> 0 Then
						If r\RoomDoorsOpt[i]\dir <> 1 Then EntityAlpha r\RoomDoorsOpt[i]\frameobj, 0.0
					EndIf
					If r\RoomDoorsOpt[i]\obj2 <> 0 Then EntityAlpha r\RoomDoorsOpt[i]\obj2, 0.0
					If r\RoomDoorsOpt[i]\buttons[0] <> 0 Then EntityAlpha r\RoomDoorsOpt[i]\buttons[0], 0.0
					If r\RoomDoorsOpt[i]\buttons[1] <> 0 Then EntityAlpha r\RoomDoorsOpt[i]\buttons[1], 0.0
				EndIf
			EndIf
		Next
		r\HidedDoors = True
...
Function ShowRoomDoors(r.Rooms)
	If r\HidedDoors Then
		For i = 1 To r\RoomDoorsCount
			If r\RoomDoorsOpt[i]<>Null Then
				If r\RoomDoorsOpt[i]\obj <> 0 Then EntityAlpha r\RoomDoorsOpt[i]\obj, 1.0
				If r\RoomDoorsOpt[i]\frameobj <> 0 Then
					If r\RoomDoorsOpt[i]\dir <> 1 Then EntityAlpha r\RoomDoorsOpt[i]\frameobj, 1.0
				EndIf
				If r\RoomDoorsOpt[i]\obj2 <> 0 Then EntityAlpha r\RoomDoorsOpt[i]\obj2, 1.0
				If r\RoomDoorsOpt[i]\buttons[0] <> 0 Then EntityAlpha r\RoomDoorsOpt[i]\buttons[0], 1.0
				If r\RoomDoorsOpt[i]\buttons[1] <> 0 Then EntityAlpha r\RoomDoorsOpt[i]\buttons[1], 1.0
			EndIf
		Next
		r\HidedDoors = False
	EndIf
	...
	If room<>Null Then
		For i = 0 To MaxRoomLights-1
			If room\Lights[i]=0 Then
				...
				PositionEntity(room\Lights[i],x,y,z)
				...
				ScaleSprite(room\LightSprites2[i], 0.3, 0.3)
		...
		If s\room = PlayerRoom Then
			If EntityDistance(Collider,s\obj)<1.2 Then
				...
				If PickedEntity()=s\obj And s\imgpath<>"" Then
					If NetworkServer\Breach And ((Not multiplayer_IsASCP(MyPlayer\BreachType)) Or multiplayer_breach_IsA049(MyPlayer\BreachType)) And PlayerRoom\RoomTemplate\Name = "room2ccont" And (Not player_isdead()) Then PlayerIntercom\Picked = True
					DrawHandIcon=True
					If MouseHit1 Then 
						If PlayerRoom\RoomTemplate\Name = "room2ccont" And NetworkServer\Breach = True And ((Not multiplayer_IsASCP(MyPlayer\BreachType)) Or multiplayer_breach_IsA049(MyPlayer\BreachType)) Then
							UseIntercom()
						Else
							SelectedScreen=s
							If s\img = 0 Then
...
Function CreateSecurityCam.SecurityCams(x#, y#, z#, r.Rooms, screen% = False)
	...
	If screen Then
		...
		sc\Cam = CreateCamera()
		CameraViewport(sc\Cam, 0, 0, GetCameraQuality(CamQuality), GetCameraQuality(CamQuality))
	...
	If r<>Null Then 
		EntityParent(sc\obj, r\obj)
		AddEntityToRoomProps(r, sc\CameraObj)
		AddEntityToRoomProps(r, sc\obj)
		AddEntityToRoomProps(r, sc\MonitorObj)
	EndIf
	
	Return sc
End Function

Function UpdateSecurityCams(UpdatePart% = 0)
	RestoreSanity = True
	If Not(FPSfactor > 0 And PlayerRoom\RoomTemplate\Name <> "dimension1499") Then Return
	
	;Catch("Uncaught (UpdateSecurityCams)")
	...
	For sc.SecurityCams = Each SecurityCams
		...
		Else
				...
				If sc\Screen Then
					...
					If BlinkTimer > - 5 Then
						If EntityInView(sc\ScrObj, Camera) Then 
							sc\InSight = EntityVisible(Camera,sc\ScrObj)
							sc\SeeMonitor = sc\InSight
						Else
							sc\InSight = False
							sc\SeeMonitor = False
						EndIf
					...
					If Not UpdatePart Then
						If (sc\State >= sc\RenderInterval)
							;sc\InSight = False
							If sc\SeeMonitor Then
								If CoffinCam = Null Or Rand(5)=5 Or sc\CoffinEffect <> 3 Then
									...
									SetBuffer BackBuffer()
									RenderWorld
									CopyRect 0,0,GetCameraQuality(CamQuality),GetCameraQuality(CamQuality),0,0,BackBuffer(),TextureBuffer(ScreenTexs[sc\ScrTexture])
			...
			If sc<>Null Then
				If sc\room<>Null Then
					;Catch("UpdateSecurityCameras ("+sc\room\RoomTemplate\Name+")")
				Else
					;Catch("UpdateSecurityCameras (screen has no room)")
				EndIf
			Else
				;Catch("UpdateSecurityCameras (screen doesn't exist anymore)")
			EndIf
		EndIf
	Next
...
Function UpdateLever(obj, locked=False)
	...
	EntityPickMode obj, 0, False
	If dist < 0.8 And (Not locked) Then
		If (Not multiplayer_IsASCP(MyPlayer\BreachType)) Or MyPlayer\BreachType = MODEL_049 Then
			If EntityInView(obj, Camera) Then 
				EntityPickMode obj, 1, False
	...
	If MouseDown1=False And MouseHit1=False Then GrabbedEntity = 0
...
Function UpdateElevators#(State#, door1.Doors, door2.Doors, room1, room2, ignorerotation% = True)
	...
	If door1\open = True And door2\open = False And door1\openstate = 180 Then 
		State = -1
		door1\locked = False
		If (ClosestButton = door2\buttons[0] Or ClosestButton = door2\buttons[1]) And IsMouseUp() Then
			UseDoor(door1,True)
			MouseUp1 = False
		EndIf
	...
	ElseIf door2\open = True And door1\open = False And door2\openstate = 180 Then
		State = 1
		door2\locked = False
		If (ClosestButton = door1\buttons[0] Or ClosestButton = door1\buttons[1]) And IsMouseUp() Then
			UseDoor(door2,True)
			MouseUp1 = False
		EndIf
			...
			If State < 0 Then
				...
				If Abs(EntityX(Collider)-EntityX(room1,True))<280.0*RoomScale+(0.015*FPSfactor) Then
					If Abs(EntityZ(Collider)-EntityZ(room1,True))<280.0*RoomScale+(0.015*FPSfactor) Then	
						If Abs(EntityY(Collider)-EntityY(room1,True))<280.0*RoomScale+(0.015*FPSfactor) Then	
							...
							If door1\liftchn = 0 Then
								door1\liftchn = PlaySound_Strict(ElevatorMoveSFX)
							Else
								If (Not ChannelPlaying(door1\liftchn)) Then door1\liftchn = PlaySound_Strict(ElevatorMoveSFX)
							EndIf
				...
				If State < -500 Then
					...
					If inside Then
						...
						;multiplayer_WriteSound(OpenDoorSFX(3, sound), EntityX(Collider), EntityY(Collider), EntityZ(Collider))
					EndIf
					...
					If Not networkserver\noclipanticheat Then
						door2\teleportedobject = False
						door1\teleportedobject = False
					EndIf
				EndIf
			Else
				State = State + FPSfactor
...
Function AddEntityToRoomProps(r.Rooms, Entity)
	If Entity <> 0 Then
		r\PropCount = r\PropCount+1
		r\Prop[r\PropCount] = Entity
		Return r\PropCount
	EndIf
	Return 0
End Function
...
Function CreateMap(SizeReduce = False)
	...
	DebugLog ("Generating a map using the seed "+RandomSeed+" | map size: "+sizereduce)
