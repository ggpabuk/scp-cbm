...
Function SaveGame(file$)
	...
	For r.Rooms = Each Rooms
		...
		Else 
			WriteByte f, 0
		EndIf
		For AA = 0 To MaxRoomObjects-1
			WriteByte f, r\ObjectStatic[AA]
		Next
		For i = 0 To 11
...
Function LoadGame(file$)
	...
	SelectedDifficulty = difficulties(ReadByte(f))
...
;        EndIf
	...
	If ReadInt(f) <> 4559 Then RuntimeError("Couldn't load the game, save file corrupted (error 113)")
	...
	if difficultyindex >= 0 and difficultyindex <= 3 then
...
Function LoadSaveGames()
	...
	For i = 1 To SaveGameAmount
		...
		ReadFloat(f)
		;End Skip
		SaveGameVersion(i - 1) = ReadFloat(f)
		SaveGameSeed(i - 1) = ReadString(f)
		SaveGameDifficulty(i - 1) = ReadByte(f)
		CloseFile f
	Next
...
Function LoadMap(file$)
	...
	Dim MapTemp%(MapWidth+2, MapHeight+2)
	Dim MapFound%(MapWidth+2, MapHeight+2)
	CoffinDistance = 100
