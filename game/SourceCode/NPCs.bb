...
Type NPCs
	...
	Field NearObject, NearID, UpdateNPC, waitFactor#
	Field ObjName$
	Field IsSpawned
	Field TimerFor
	Field EventID, NPCEventID
	; --------------------- 0.5 version
	Field PLAYER_MOVE, StupidMode
	Field SoundChn3
	; --------------------- 0.69 version
	Field x#, y#, z#, smoothx#, smoothy#, smoothz#
	Field mpFrame#, yaw#, smoothyaw#, smoothframe#
	; --------------------- 0.8 version
	Field statemultiplayer#, channelmultiplayer%
	; --------------------- 1.0 version
	Field prevnearid%
End Type
Global M_NPC.NPCs[MAX_NPCS]
...
Function CreateNPC.NPCs(NPCtype%, x#, y#, z#)
	...
	DebugLog ("Created NPC "+n\NVName+" (ID: "+n\ID+")"+" Type = "+n\NPCtype)
	
	NPCSpeedChange(n)
	NPCcount = NPCcount+1
	Return n
End Function
Function FillNPC(n.NPCs, NPCtype)
	Select NPCtype
		Case NPCtype173
			;[Block]
			n\NVName = "SCP-173"
		...
		Case NPCtype008
			...
			n\obj = CopyEntity(SURGEON_ZOMBIE)
			NameEntityEx(n, "008")
			temp# = 0.5 / MeshWidth(n\obj)
			ScaleEntity n\obj, temp, temp, temp
	...
	End Select
End Function
Function RemoveNPC(n.NPCs, sync = False)
...
Function UpdateNPCs()
	...
	if NetworkServer\Breach = True Then
		...
		Return
	EndIf
	Local n2.NPCs, d.Doors, de.Decals, r.Rooms
	...
	For n.NPCs = Each NPCs
		...
		n\prevnearid = n\NearID
		n\NearID = FindNearestID(n)
		n\NearObject = Player[n\NearID]\Pivot
		Select n\NPCtype
			Case NPCtype173
				;[Block]
				SetAnimTime(n\obj, 0)
				If n\Idle <> 3 Then
					dist# = EntityDistance(n\Collider, n\NearObject)		
					...
					If n\Idle < 2 Then
						...
						If n\Idle = False Then
							...
							If dist < 15 Then
								If dist < 10.0 Then 
									If EntityVisible(n\Collider, n\NearObject) Then
										temp = True
										n\EnemyX = EntityX(n\NearObject, True)
										n\EnemyY = EntityY(n\NearObject, True)
										n\EnemyZ = EntityZ(n\NearObject, True)
									EndIf
								EndIf										
								...
								For pl.players = Each players
									If (pl\BlinkTimer < - 16 Or pl\BlinkTimer > - 6) Then
										If EntityInView(n\obj,GetPlayerCamera(pl\ID)) Then move = False
									EndIf
								Next
							EndIf
							...
							If move=False Then
								if n\NearID = NetworkServer\MyID Then
							...
							Else 
								...
								If dist > 50 Then
									If Rand(70)=1 Then
										If Player[n\NearID]\PlayerRoomName <> "exit1" And Player[n\NearID]\PlayerRoomName <> "gatea" And Player[n\NearID]\PlayerRoomName <> "pocketdimension" Then
											For w.waypoints = Each WayPoints
												If w\door=Null And Rand(5)=1 Then
													x = Abs(EntityX(n\NearObject)-EntityX(w\obj,True))
													If x < 25.0 And x > 15.0 Then
														z = Abs(EntityZ(n\NearObject)-EntityZ(w\obj,True))
								...
								Else ;less than 3 rooms away -> actively move towards the player
									...
									If temp Then 				
										If dist < 0.65 Then
											If KillTimer >= 0 And (Not GodMode) And n\NearID = NetworkServer\MyID Then
												...
												multiplayer_WriteSound(NeckSnapSFX(Rand(0,2)), EntityX(Collider),EntityY(Collider),EntityZ(Collider))
												If Rand(2) = 1 Then 
													TurnEntity(Camera, 0, Rand(80,100), 0)
												Else
													TurnEntity(Camera, 0, Rand(-100,-80), 0)
												EndIf
												Kill("was killed by SCP-173")
												
											Else
												GiveDamage(n\NearID, -1)
											EndIf
										Else
											PointEntity(n\Collider, n\NearObject)
					...
					Else ;idle = 2
						...
						If n\Target <> Null Then
							...
							If dist > HideDistance*0.7
								If EntityVisible(n\obj,n\NearObject)=False
									tmp = True
								EndIf
							EndIf
							If (Not tmp)
			...
			Case NPCtypeOldMan ;------------------------------------------------------------------------------------------------------------------
				...
				Else
					...
					if Player[n\NearID]\PlayerRoomName <> "pocketdimension" Then
						if pocketdimension106\nearobjectqueue > 0 then
							if PlayerInRoom(pocketdimension106) Then spawn106 = False
						endif
					Else
						if EntityDistance(n\NearObject, n\collider) > 50 Then spawn106 = False
					EndIf
					
					If Player[n\NearID]\PlayerRoomName = "dimension1499" Then spawn106% = False
					
					if room860event <> Null Then
						If room860event\EventState = 1 Then spawn106% = False
					EndIf
					...
					if gateaevent <> Null Then
						If gateaevent\EventState <> 0
							spawn106% = True
							If Player[n\NearID]\PlayerRoomName = "dimension1499" Then
								n\Idle = True
							Else
								n\Idle = False
							EndIf
					...
					If (Not n\Idle) And spawn106%
						If n\State <= 0 Then	;attacking	
							If EntityY(n\Collider) < EntityY(n\NearObject) - 20.0 - 0.55 And (Not ChannelPlaying(n\channelmultiplayer)) Then
								if Room[Player[n\NearID]\PlayerRoomID] <> Null Then
									If Not Room[Player[n\NearID]\PlayerRoomID]\RoomTemplate\DisableDecals Then
										de.Decals = CreateDecal(0, EntityX(n\NearObject), 0.01, EntityZ(n\NearObject), 90, Rand(360), 0)
										de\Size = 0.05 : de\SizeChange = 0.001 : EntityAlpha(de\obj, 0.8) : UpdateDecals
									EndIf
								EndIf
								n\PrevY = EntityY(n\NearObject)
								
								SetAnimTime n\obj, 110
								
								If Player[n\NearID]\PlayerRoomName <> "coffin"
									PositionEntity(n\Collider, EntityX(n\NearObject), EntityY(n\NearObject) - 15, EntityZ(n\NearObject))
								EndIf
								
								if EntityDistance(n\Collider, Collider) < 8 Then n\channelmultiplayer = PlaySound_Strict(DecaySFX(0))
							End If
							...
							If n\State > - 10 Then
								if EntityDistance(n\Collider, Collider) < 8 Then ShouldPlay = 66
								If n\Frame<259 Then
									PositionEntity n\Collider, EntityX(n\Collider), n\PrevY-0.15, EntityZ(n\Collider)
									PointEntity n\obj, n\NearObject
								...
								EndIf
							Else
								If PlayerRoom\RoomTemplate\Name <> "gatea" And EntityDistance(n\Collider, Collider) < 8 Then ShouldPlay = 10
								
								Local Visible% = False
								If dist < 8.0 Then
									Visible% = EntityVisible(n\Collider, n\NearObject)
								EndIf
								
								If NoTarget Then Visible = False
								
								If Visible Then
									If Player[n\NearID]\PlayerRoomName <> "gatea" Then n\PathTimer = 0
								...
								If dist > 0.8 Then
									If (dist > 25.0 Or Player[n\NearID]\PlayerRoomName = "pocketdimension" Or Visible Or n\PathStatus <> 1) And Player[n\NearID]\PlayerRoomName <> "gatea" Then 
										
										If (dist > 40 Or Player[n\NearID]\PlayerRoomName = "pocketdimension") Then
											TranslateEntity n\Collider, 0, ((EntityY(n\NearObject) - 0.14) - EntityY(n\Collider)) / 50.0, 0
										EndIf
										...
										If n\PathTimer =< 0 Then
											n\PathStatus = FindPath (n, EntityX(n\NearObject,True), EntityY(n\NearObject,True), EntityZ(n\NearObject,True))
											n\PathTimer = 70*10
										EndIf
									Else 
										If n\PathTimer <= 0 Then
								...
								ElseIf Player[n\NearID]\PlayerRoomName <> "gatea" And (Not NoTarget) ;dist < 0.8
									...
									if n\NearID = NetworkServer\MyID Then
										...
										If KillTimer >= 0 And FallTimer >= 0 Then								
											If Ceil(n\Frame) = 110 And (Not GodMode) Then
												...
												multiplayer_WriteSound(DamageSFX(1),EntityX(Collider),EntityY(Collider),EntityZ(Collider))
												If Player[n\NearID]\PlayerRoomName = "pocketdimension" Then
							...
							If n\State <= Rand(-3500, -3000) Then 
								If Not EntityInView(n\obj,GetPlayerCamera(n\NearID)) And dist > 5 Then
									n\State = Rand(22000, 27000)
									PositionEntity n\Collider,0,500,0
								EndIf
							EndIf
							...
							If n\Reload = 0 Then
                                If dist > 10 And Player[n\NearID]\PlayerRoomName <> "pocketdimension" And Player[n\NearID]\PlayerRoomName <> "gatea" And n\State <-5 Then ;timer idea by Juanjpro
                                    If (Not EntityInView(n\obj,GetPlayerCamera(n\NearID)))
                                        TurnEntity n\NearObject,0,180,0
                                        pick = EntityPick(n\NearObject,5)
                                        TurnEntity n\NearObject,0,180,0
                                        If pick<>0
                                            ...
                                            DebugLog "Teleported 106 (Distance: "+EntityDistance(n\Collider,n\NearObject)+")"
                                        EndIf
                                    EndIf
                                EndIf
                            EndIf
						...
						Else ;idling outside the map
							n\CurrSpeed = 0
							MoveEntity n\Collider, 0, ((EntityY(n\NearObject) - 30) - EntityY(n\Collider)) / 200.0, 0
							n\DropSpeed = 0
							n\Frame = 110
							;SetAnimTime n\obj, 110.0
							if Room[Player[n\NearID]\PlayerRoomID] <> Null Then
								If (Not Room[Player[n\NearID]\PlayerRoomID]\RoomTemplate\DisableDecals) Then
									If Player[n\NearID]\PlayerRoomName <> "gatea" Then
						...
						If PlayerRoom\RoomTemplate\name = "pocketdimension" Or PlayerRoom\RoomTemplate\name = "gatea" Then
							HideEntity n\obj2
						Else
							If EntityDistance(Camera, n\Collider) < CameraFogFar*LightVolume*0.6 Then
								HideEntity n\obj2
							Else
								ShowEntity n\obj2
			...
			Case NPCtype096
				;[Block]
				dist = EntityDistance(n\NearObject, n\Collider)
				...
				Select n\State
					Case 0
									...
									For i = 1 To NetworkServer\maxplayers
										if Player[i] <> Null Then
											if Not Player[i]\IsDead Then
												angle# = WrapAngle(DeltaYaw(n\Collider, GetPlayerCamera(i))) ;-EntityYaw(n\Collider,True))
												If angle<90 Or angle>270 Then
													
													CameraProject GetPlayerCamera(i),EntityX(n\Collider), EntityY(n\Collider)+0.25, EntityZ(n\Collider)
													If ProjectedX()>0 And ProjectedX()<Player[i]\monitor_width Then
														If ProjectedY()>0 And ProjectedY()<Player[i]\monitor_height Then
															If EntityVisible(Player[i]\Pivot, n\Collider) Then
																If (Player[i]\BlinkTimer < - 16 Or Player[i]\BlinkTimer > - 6) Then
																	if (Not ChannelPlaying(n\SoundChn3)) Then n\SoundCHN3 = PlaySound_Strict(Triggered096SFX)
						...
						Else
							If n\SoundChn <> 0 Then
								StopStream_Strict(n\SoundChn)
								n\SoundChn_IsStream = False
								n\SoundChn = 0
							EndIf
							If n\SoundChn2 <> 0 Then
								StopStream_Strict(n\SoundChn2)
								n\SoundChn2_IsStream = 0
								n\SoundChn2 = 0
							EndIf
						EndIf
						;[End Block]
					Case 4
						...
						If MilliSecs2() > n\StateMultiplayer Then
							n\LastSeen=0
							If n\Target=Null Then
								If EntityVisible(n\Collider, n\NearObject) Then n\LastSeen=1
							Else
								If EntityVisible(n\Target\Collider, n\Collider) Then n\LastSeen=1
							EndIf
							n\StateMultiplayer=MilliSecs2()+3000
						EndIf
						...
						If n\LastSeen=1 Then
							...
							if n\prevnearid <> 0 then dist = EntityDistance(Player[n\prevnearid]\Pivot, n\Collider)
							If dist < 2.8 Or n\Frame<150 Then 
								...
								Else
									...
									If n\Target=Null Then
										If (Not GodMode) Then 
											if n\NearID = NetworkServer\MyID Then
												if not player_isdead() then
													...
													multiplayer_WriteSound(DamageSFX(4), EntityX(Collider),EntityY(Collider),EntityZ(Collider))
													pvt = CreatePivot()
													CameraShake = 30
													BlurTimer = 2000
													...
													Kill("was killed by SCP-096")
													KillAnim = 1
													For i = 0 To 6
														PositionEntity pvt, EntityX(n\NearObject)+Rnd(-0.1,0.1),EntityY(n\NearObject)-0.05,EntityZ(n\NearObject)+Rnd(-0.1,0.1)
										...
										if Player[n\PrevNearID]\IsDead Then 
											n\State = 0
											If n\SoundChn <> 0 Then
												StopStream_Strict(n\SoundChn)
												n\SoundChn_IsStream = False
						...
						Else
							...
							Else
								...
								If n\PathTimer=<0 Then
									...
									Else
										n\PathStatus = FindPath(n, EntityX(n\NearObject),EntityY(n\NearObject)+0.2,EntityZ(n\NearObject))	
									EndIf
									n\PathTimer = 70*5
								EndIf
							EndIf
						...
						If dist < 16.0 Then 
							...
							If n\Frame>=422
								...
								If n\State2>1000 Then ;walking around
									...
									If MilliSecs2() > n\StateMultiplayer Then
										n\LastSeen=0
										If EntityVisible(n\NearObject, n\Collider) Then 
											n\LastSeen=1
										Else
											HideEntity n\Collider
							...
							if NetworkServer\MainPlayer Then
								If (Not NoTarget)
									For i = 1 To NetworkServer\Maxplayers
										if Player[i] <> Null Then
											if Not Player[i]\IsDead Then
												angle = WrapAngle(DeltaYaw(n\Collider, GetPlayerCamera(i)))
												If angle<55 Or angle>360-55 Then
													CameraProject GetPlayerCamera(i),EntityX(n\Collider), EntityY(Player[i]\Pivot)+5.8*0.2-0.25, EntityZ(n\Collider)
													If ProjectedX()>0 And ProjectedX()<Player[i]\monitor_width Then
														If ProjectedY()>0 And ProjectedY()<Player[i]\monitor_height Then
															If EntityVisible(Player[i]\Pivot, n\Collider) Then
																If (Player[i]\BlinkTimer < - 16 Or Player[i]\BlinkTimer > - 6) Then
			...
			Case NPCtype049
				...
				If n\Idle > 0.1
					If Player[n\NearID]\PlayerRoomName <> "room049"
				...
				Else
					...
					If n\Idle = 0.1 Then
						If OtherPlayerInReachableRoom(n\NearID) Then
							if Room[Player[n\NearID]\PlayerRoomID] <> Null Then
								For i = 0 To 3
									If Room[Player[n\NearID]\PlayerRoomID]\Adjacent[i]<>Null Then
										For j = 0 To 3
											If Room[Player[n\NearID]\PlayerRoomID]\Adjacent[i]\Adjacent[j]<>Null Then
												TeleportEntity(n\Collider,Room[Player[n\NearID]\PlayerRoomID]\Adjacent[i]\Adjacent[j]\x,0.5,Room[Player[n\NearID]\PlayerRoomID]\Adjacent[i]\Adjacent[j]\z,n\CollRadius,True)
												Exit
											EndIf
										Next
										Exit
					...
					Select n\State
						...
						Case 2 ;being active
							;[Block]
							If (dist < HideDistance*2) And (Not n\Idle) Then
								...
								If PlayerSeeAble%=True Or n\State2>0 Then ;Player is visible for 049's sight - attacking
									...
									If n\PrevState <= 1 And ChannelPlaying(n\SoundChn2)=False
										If n\Sound2 <> 0 Then FreeSound_Strict(n\Sound2)
										syncRand = Rand(1,7)
										n\Sound2 = LoadSound_Strict("SFX\SCP\049\Spotted"+syncRand+".ogg")
										;multiplayer_WriteTempSound("SFX\SCP\049\Spotted"+syncRand+".ogg", EntityX(n\obj),EntityY(n\obj),EntityZ(n\obj))
										...
										if n\NearID = NetworkServer\MyID Then
											If WearingHazmat>0 Then
												...
												If BlurTimer>250 And BlurTimer-FPSfactor*2.5 <= 250 And n\PrevState<>3 Then
													...
													;multiplayer_WriteTempSound("SFX\SCP\049\TakeOffHazmat.ogg", EntityX(n\obj),EntityY(n\obj),EntityZ(n\obj))
													n\PrevState=3
											...
											ElseIf Wearing714 Then
												...
												If BlurTimer>250 And BlurTimer-FPSfactor*2.5 <= 250 And n\PrevState<>3 Then
													...
													;multiplayer_WriteTempSound("SFX\SCP\049\714Equipped.ogg", EntityX(n\obj),EntityY(n\obj),EntityZ(n\obj))
													n\PrevState=3
												ElseIf BlurTimer => 500
													Wearing714=False
												EndIf
											Else
												...
												If (Not GodMode) And KillTimer >= 0 And FallTimer >= 0 Then
													DeathMSG = "An active instance of SCP-049-2 was discovered in [REDACTED]. Terminated by Nine-Tailed Fox."
													Kill("was killed by SCP-049")
													PlaySound_Strict HorrorSFX(13)
													If n\Sound2 <> 0 Then FreeSound_Strict(n\Sound2)
													syncRand = Rand(1,2)
													n\Sound2 = LoadSound_Strict("SFX\SCP\049\Kidnap"+syncRand+".ogg")
													n\SoundChn2 = LoopSound2(n\Sound2,n\SoundChn2,Camera,n\obj)
													;multiplayer_WriteTempSound("SFX\SCP\049\Kidnap"+syncRand+".ogg", EntityX(n\obj),EntityY(n\obj),EntityZ(n\obj))
													n\State = 1
												EndIf										
											EndIf
										Else
								...
								Else ;Finding a path to the player
										...
										If n\Path[n\PathLocation]<>Null Then
													...
													If (Not n\Path[n\PathLocation]\door\IsElevatorDoor)
														...
														Else
															If n\Path[n\PathLocation]\door\open = False And (n\Path[n\PathLocation]\door\buttons[0]<>0 Or n\Path[n\PathLocation]\door\buttons[1]<>0) Then
																if NetworkServer\MainPlayer Then UseDoor(n\Path[n\PathLocation]\door, False, True, False)
															EndIf
														EndIf
													EndIf
												EndIf
											...
											If n\PrevState = 0 And ChannelPlaying(n\SoundChn2)=False
												...
												If Rand(30)=1
													...
													;multiplayer_WriteTempSound("SFX\SCP\049\Searching7.ogg", EntityX(n\obj), EntityY(n\obj), EntityZ(n\obj))
												Else
													syncRand = Rand(1,6)
													n\Sound2 = LoadSound_Strict("SFX\SCP\049\Searching"+syncRand+".ogg")
												;	multiplayer_WriteTempSound("SFX\SCP\049\Searching"+syncRand+".ogg", EntityX(n\obj), EntityY(n\obj), EntityZ(n\obj))
												EndIf
												n\SoundChn2 = LoopSound2(n\Sound2,n\SoundChn2,Camera,n\obj)
												n\PrevState = 1
											EndIf
									...
									Else ;No Path to the player found - stands still and tries to find a path
										...
										If n\PathTimer# > 70*(5-(2*SelectedDifficulty\aggressiveNPCs)) Then
											n\PathStatus = FindPath(n, EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject))
											n\PathTimer# = 0.0
											n\State3 = 0
											
											;Attempt to find a room (the Playerroom or one of it's adjacent rooms) for 049 to go to but select the one closest to him
											If n\PathStatus <> 1 Then
												Local closestdist# = EntityDistance(Room[Player[n\NearID]\PlayerRoomID]\obj,n\Collider)
												Local closestRoom.Rooms = Room[Player[n\NearID]\PlayerRoomID]
												Local currdist# = 0.0
												
												For i = 0 To 3
													If Room[Player[n\NearID]\PlayerRoomID]\Adjacent[i]<>Null Then
														currdist = EntityDistance(Room[Player[n\NearID]\PlayerRoomID]\Adjacent[i]\obj,n\Collider)
														If currdist < closestdist Then
															closestdist = currdist
															closestRoom = Room[Player[n\NearID]\PlayerRoomID]\Adjacent[i]
														EndIf
													EndIf
												Next
													...
													For i = 0 To 3
														If Room[Player[n\NearID]\PlayerRoomID]\Adjacent[i]<>Null Then
															...
															If currdist < closestdist Then
																...
																For j = 0 To 3
																	If Room[Player[n\NearID]\PlayerRoomID]\Adjacent[i]\Adjacent[j]<>Null Then
																		If Room[Player[n\NearID]\PlayerRoomID]\Adjacent[i]\Adjacent[j]<>Room[Player[n\NearID]\PlayerRoomID] Then
																			closestRoom = Room[Player[n\NearID]\PlayerRoomID]\Adjacent[i]\Adjacent[j]
																			Exit
																		EndIf
																	EndIf
																Next
							...
							ElseIf (Not n\Idle)
								If ChannelPlaying(n\SoundChn) Then
									StopChannel(n\SoundChn)
								EndIf
								If OtherPlayerInReachableRoom(n\NearID, True) And InFacility=1 Then ;Player is in a room where SCP-049 can teleport to
						...
						Case 3 ;The player was killed by SCP-049
							...
							PositionEntity n\Collider, CurveValue(EntityX(n\NearObject),EntityX(n\Collider),20.0),EntityY(n\Collider),CurveValue(EntityZ(n\NearObject),EntityZ(n\Collider),20.0)
							RotateEntity n\Collider, 0, CurveAngle(EntityYaw(n\NearObject)-180.0,EntityYaw(n\Collider),40), 0
							;[End Block]
						...
						Case 5 ;used for "room2sl"
							...
							Else
								If n\State3 = 6.0
									If EntityDistance(n\Collider,n\NearObject)>HideDistance
										n\State = 2
										n\PathStatus = 0
										n\PathLocation = 0
										n\PathTimer = 0
										...
										n\PrevState = 0
									Else
										If n\PathStatus <> 1 Then n\PathStatus = FindPath(n,EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject))
									EndIf
								EndIf
												...
												If n\Path[n\PathLocation-1]\door <> Null Then
													If n\Path[n\PathLocation-1]\door\KeyCard=0
														If EntityDistance(n\Path[n\PathLocation-1]\obj,n\Collider)>0.3
															If n\Path[n\PathLocation-1]\door\open Then 
																if NetworkServer\MainPlayer Then UseDoor(n\Path[n\PathLocation-1]\door, False, True, False)
															EndIf
														EndIf
													EndIf
												EndIf
			...
			Case NPCtypeZombie
				...
				If Abs(EntityY(n\NearObject)-EntityY(n\Collider))<4.0 Then
					prevFrame# = n\Frame
					n\isdead = (n\mpframe >= 133 And n\Mpframe <= 157)
					If (Not n\IsDead)
						Select n\State
							Case 0
								;[Block]
								...
								If n\Frame=777 Then
									If Rand(700)=1 Then
										If EntityDistance(n\NearObject, n\Collider)<5.0 Then
											n\Frame = 719
										EndIf
									EndIf
								EndIf
							...
							Case 2 ;following the player
								...
								If n\State3 < 0 Then ;check if the player is visible every three seconds
									If EntityDistance(n\NearObject, n\Collider)<5.0 Then 
										If EntityVisible(n\Collider, n\NearObject) Then n\State2 = 70*5
									EndIf
									n\State3=70*3
								Else
									n\State3=n\State3-FPSfactor
								...
								Else
									...
									Else ;no path to the player, stands still
										...
										n\PathTimer = n\PathTimer-FPSfactor
										If n\PathTimer =< 0 Then
											n\PathStatus = FindPath(n, EntityX(n\NearObject),EntityY(n\NearObject)+0.1,EntityZ(n\NearObject))
							...
							Case 3
								...
								If n\Frame < 66 Then
									...
									If prevFrame < 23 And n\Frame=>23 Then
										If EntityDistance(n\Collider,n\NearObject)<1.1
											If (Abs(DeltaYaw(n\Collider,n\NearObject))<=60.0) Then
												if n\NearID = NetworkServer\MyID Then
													PlaySound_Strict DamageSFX(Rand(5,8))
													multiplayer_WriteSound(DamageSFX(Rand(5,8)), EntityX(Collider), EntityY(Collider), EntityZ(Collider), 5.0)
												...
												Else
													GiveDamage(n\NearID, Rnd(0.4,1.0))
												EndIf
											EndIf
										EndIf
			...
			Case NPCtypeGuard ;------------------------------------------------------------------------------------------------------------------
				...
				;if n\TimerFor < MilliSecs() Then
				;	DebugLog "Near Name = "+Player[n\NearID]\name
				;	n\TimerFor = MilliSecs()+1000
				;EndIf
				Select n\State
						...
						If KillTimer => 0 Then
							...
							If dist < DetectDistance Then
								...
								If n\Reload = 0 ;And n\Frame>1550 Then
									DebugLog "entitypick"
									EntityPick(pvt, dist)
									If PickedEntity() = n\NearObject Or n\State3=1 Then
										...
										PointEntity pvt, n\NearObject
										Shoot(n,EntityX(pvt), EntityY(pvt), EntityZ(pvt), ShootAccuracy, False, instaKillPlayer)
										n\Reload = 7
									Else
										n\CurrSpeed = n\Speed
									End If
					...
					Case 3 ;follows a path
						...
						ElseIf n\PathStatus = 1
							If n\Path[n\PathLocation]=Null Then 
								...
								EndIf
							Else
								DebugLog "EntityX = "+EntityX(n\Path[n\PathLocation]\obj)
								DebugLog "EntityY = "+EntityY(n\Path[n\PathLocation]\obj)
								DebugLog "EntityZ = "+EntityZ(n\Path[n\PathLocation]\obj)
								
								DebugLog "EntityX = "+EntityX(n\Path[n\PathLocation]\obj, True)
								DebugLog "EntityY = "+EntityY(n\Path[n\PathLocation]\obj, True)
								DebugLog "EntityZ = "+EntityZ(n\Path[n\PathLocation]\obj, True)
								PointEntity n\obj, n\Path[n\PathLocation]\obj
					...
					Case 4
						...
						If dist < 15.0 Then
							...
							If WrapAngle(EntityYaw(n\Collider)-DeltaYaw(n\Collider, n\NearObject))<90 Then
								If EntityVisible(n\Collider,n\NearObject) Then n\State = 1
							EndIf
					...
					Case 11
						...
						If KillTimer => 0 Then
							...
							If dist < 11.0
								If EntityVisible(n\Collider,n\NearObject)
									SearchPlayer = True
								EndIf
							EndIf
							...
							If SearchPlayer
								...
								If n\Reload = 0
									...
									If PickedEntity() = n\NearObject Or n\State3=1 Then
										...
										PointEntity pvt, n\NearObject
										Shoot(n,EntityX(pvt), EntityY(pvt), EntityZ(pvt), 1.0, False, instaKillPlayer)
										n\Reload = 7
									Else
										n\CurrSpeed = n\Speed
									End If
							...
							Else
								If n\PathStatus = 1
									...
									EndIf
								Else
									If n\PathTimer = 0 Then n\PathStatus = FindPath(n,EntityX(n\NearObject),EntityY(n\NearObject)+0.5,EntityZ(n\NearObject))
									
									Local wayPointCloseToPlayer.WayPoints
									wayPointCloseToPlayer = Null
									
									For wp.WayPoints = Each WayPoints
										If EntityDistance(wp\obj,n\NearObject)<2.0
			...
			Case NPCtypeMTF ;------------------------------------------------------------------------------------------------------------------
				...
				;[End Block]
			;Case NPCtypeClassDBot
				;Local botid = GetBotByID(n\ID)
				;Player[botid]\Timeout = MilliSecs()+15000
				;Player[botid]\BlinkTimer = -15
				...
				;			If Rand(1000) = 5 Then 
				;				RotateEntity(n\Collider, 0, Rand(360), 0, True)
				;				n\Angle = EntityYaw(n\Collider)
				;				n\State = Rand(1,4)
				;				n\PathTimer = 0.0
				;				n\PathStatus = 0
				;			EndIf
				;			If Rand(200) = 170 Then 
				;				n\State = Rand(1,4)
				;				n\PathTimer = 0.0
				;				n\PathStatus = 0
				;				n\currspeed = 0
				;			EndIf
				;			if Rand(300) = 250 Then
				;				RotateEntity n\Collider, 0, EntityYaw(Collider)-180, 0, True
				;				n\Angle = EntityYaw(n\Collider)
				;				if EntityDistance(n\NearObject, n\Collider) < 8 Then 
				;					Play3DSound(0, Camera, n\Collider, 8, 1, "SFX\multiplayer\scream.mp3")
				;					multiplayer_WriteTempSound("SFX\multiplayer\scream.mp3", 0,0,0,8,1,botid)
				;				EndIf
				;			EndIf
				;			n\player_move = PLAYER_IDLING
				;		Case 1 ;walking
				...
				;				EndIf
				;			EndIf
				;			n\PLAYER_MOVE = PLAYER_RUNNING
				;		Case 3; sitting idle
				;			n\CurrSpeed = CurveValue(0.0, n\CurrSpeed, 5.0)
				;			Animate2(n\obj, AnimTime(n\obj), 357, 381, 0.1)
				;			If Rand(1000) = 5 Then 
				;				RotateEntity(n\Collider, 0, Rand(360), 0, True)
				;				n\Angle = EntityYaw(n\Collider)
				;				n\State = Rand(1,4)
				...
				;				EndIf
				;			EndIf
				;			n\PLAYER_MOVE = PLAYER_SITTING_IDLING
				;		Case 4 ; sitting walking
				;			n\player_move = PLAYER_SITTING_WALKING_FORWARD
				;			n\CurrSpeed = CurveValue(0.015/1.5, n\CurrSpeed, 5.0)
				;			Animate2(n\obj, AnimTime(n\obj), 382, 406, n\CurrSpeed * 18)
				;			If Rand(1000) = 5 Then 
				;				n\State = 0
				;				n\PathTimer = 0.0
				;				n\PathStatus = 0
				...
				;				multiplayer_WriteSound(StepSFX(GetStepSound(n\Collider),1,Rand(0,2)),0,0,0,8.0, Rnd(0.3,0.5), botid)
				...
				;	ElseIf n\State = 4
				;		If n\CurrSpeed > 0.01 Then
				;			If prevFrame < 390 And AnimTime(n\obj)=>390
				;				PlaySound2(StepSFX(GetStepSound(n\Collider),1,Rand(0,2)),Camera, n\Collider, 8.0, Rnd(0.1,0.3))
				;				multiplayer_WriteSound(StepSFX(GetStepSound(n\Collider),1,Rand(0,2)),0,0,0,8.0, Rnd(0.1,0.3), botid)
				;			ElseIf prevFrame =< 393 And AnimTime(n\obj)=<375
				;				PlaySound2(StepSFX(GetStepSound(n\Collider),1,Rand(0,2)),Camera, n\Collider, 8.0, Rnd(0.1,0.3))
				;				multiplayer_WriteSound(StepSFX(GetStepSound(n\Collider),1,Rand(0,2)),0,0,0,8.0, Rnd(0.1,0.3), botid)
				;			EndIf
				;		EndIf
				;	EndIf
				;	If NetworkServer\MainPlayer = True And n\StupidMode = False Then
				;;;;;;;;;		If n\State > 0 And n\state < 4 Then
				;;;;;;;;			If n\PathTimer<=0.0 And n\PathStatus = 0 Then ;update path
				;;;;;;;				;Local roomexit$
				;;;;;;;;				if Rand(3) = 1 Then
				;;;;;;;;;;;;;;;					For r.Rooms = Each Rooms
				;;;;;;;;;						If Rand(50) = 5 Then
				...
				;;;;;;;;;						if p2\ID <> botid Then
				;;;;;;;;;							if Rand(3) = 1 Then 
				;;;;;;;;;;;;								if p2\ID <> NetworkServer\MyID Then
				;;;;;;;;;;									n\PathStatus = FindPath(n,EntityX(p2\Pivot), EntityY(p2\Pivot), EntityZ(p2\Pivot)) : Exit
				;;;;;;;								Else
				;;;;;;;;;;									n\PathStatus = FindPath(n,EntityX(Collider), EntityY(Collider), EntityZ(Collider)) : Exit
				;;;;;;;;;;								EndIf
				;;;;;;;;;							EndIf
				;;;;;;;;						EndIf
				;;;;;;;;;;;;;					Next
				...
				;;;;;;;;;						For n2.NPCs = Each NPCs
				;;;;;;;;;							if n <> n2 Then
				;;;;;;;								if Rand(3) = 1 Then n\PathStatus = FindPath(n,EntityX(n2\Collider), EntityY(n2\Collider), EntityZ(n2\Collider)) : Exit
				;;;;;;;;							EndIf
				;;;;;;;;						Next
				;;;;;;;;;					EndIf
				;;;;;;;				EndIf
				...
				;;;;;							If (Not n\Path[n\PathLocation]\door\open) And (Not n\Path[n\PathLocation]\door\locked) And n\Path[n\PathLocation]\door\KeyCard = 0 Then
				;;;;;;;;;;;;;;;								UseDoor(n\Path[n\PathLocation]\door, False, True, False)
				;;;;;;;;								;PlaySoundEx(ButtonSFX, Camera, n\Path[n\PathLocation]\door\obj)
				;;;;;;;;;;;;;;;;;								;multiplayer_WriteSound(ButtonSFX, EntityX(n\Path[n\PathLocation]\door\obj), EntityY(n\Path[n\PathLocation]\door\obj), EntityZ(n\Path[n\PathLocation]\door\obj), 2.3)
				;;;;;;;;;;;							EndIf
				;;;;;;;;							If (n\Path[n\PathLocation]\door\locked And n\Path[n\PathLocation]\door\open = False) Or (n\Path[n\PathLocation]\door\KeyCard < 0 Or n\Path[n\PathLocation]\door\KeyCard > 0) Then
				;;;;;;;								n\State = 0
				;;;;;;;;;;;;;;								;For r.Rooms = Each Rooms
				;;;;;;;;								;	If Rand(50) = 5 And IsAEventRoom(r\RoomTemplate\Name) Then
				;;;;;;;;;;;;								;		n\PathStatus = FindPath(n,EntityX(r\obj), EntityY(r\obj), EntityZ(r\obj))
				;;;;;;								;		;n\PathTimer = 70.0 * Rnd(6.0,10.0) ;search again after 6 seconds
				;;;;;;;;;;;;;;;;;;;;								;		Exit
				;;;;;;								;	EndIf
				...
				;;;;;;;	PositionEntity(n\obj, EntityX(n\Collider), EntityY(n\Collider) - 0.32, EntityZ(n\Collider))
				;;;;;;;		
				;	RotateEntity n\obj,EntityPitch(n\Collider),n\Angle-180,0.0,True
				;
				;EndIf
				;
				;Local headbone = GetPlayerHead(botid)
				;Player[botid]\x = EntityX(headbone,True)
				;Player[botid]\y = EntityY(headbone,True)
				;Player[botid]\z = EntityZ(headbone,True)
				;Player[botid]\yaw = EntityYaw(headbone)-180
				;ShowEntity Player[botid]\AttachObjects\NewYearHat

				;; --------- new year hat upd
				;	rd_SetForce(Player[botid]\AttachObjects\NYHatBone[0], Player[botid]\prevx-Player[botid]\x, Player[botid]\prevy-Player[botid]\y, Player[botid]\prevz-Player[botid]\z)
				;	rd_SetForce(Player[botid]\AttachObjects\NYHatBone[1], Player[botid]\prevx-Player[botid]\x, Player[botid]\prevy-Player[botid]\y, Player[botid]\prevz-Player[botid]\z)
				;	
				;	rd_SetForce(Player[botid]\AttachObjects\NYHatBone[0], 0, 0, (Player[botid]\prevyaw-Player[botid]\yaw)*0.1)
				;	rd_SetForce(Player[botid]\AttachObjects\NYHatBone[1], 0, 0, (Player[botid]\prevyaw-Player[botid]\yaw)*0.1)
				;; ---------
				;multiplayer_UpdatePlayerRagdolls(Player[botid])
				
				;Player[botid]\prevx = Player[botid]\x
				;Player[botid]\prevy = Player[botid]\y
				;Player[botid]\prevz = Player[botid]\z
				;Player[botid]\prevyaw = Player[botid]\yaw
			Case NPCtypeD,NPCtypeClerk 	;------------------------------------------------------------------------------------------------------------------
				...
				If AnimTime(n\obj)=19 Or AnimTime(n\obj)=60
					n\IsDead = True
				EndIf
				if n\IsDead Then n\CurrSpeed = 0
			...
			Case NPCtype5131
				...
				If Player[n\NearID]\PlayerRoomName <> "pocketdimension" Then 
					...
					Else
						...
						If Rand(100)=1 Then
							If Rand(5)=1 Then
								n\PrevX = (EntityX(n\NearObject)-EntityX(n\Collider))*0.9
								n\PrevZ = (EntityZ(n\NearObject)-EntityZ(n\Collider))*0.9
							Else
								n\PrevX = Rnd(0.1,0.5)
								n\PrevZ = Rnd(0.1,0.5)						
							EndIf
						...
						If n\State = 0 Then
							...
							If n\LastSeen Then 	
								PointEntity n\obj2, n\NearObject
						...
						Else
								...
								For w.WayPoints = Each WayPoints
									...
									If x < 8.0 And x > 1.0 Then
										...
										If z < 8.0 And z > 1.0 Then
											If EntityDistance(n\NearObject, w\obj) > dist Then
												n\Path[0]=w
												Exit
											EndIf
										EndIf
							...
							Else
								...
								If EntityDistance(n\Collider, n\Path[0]\obj) > 1.0 Then
									...
									MoveEntity n\Collider, 0,0,n\CurrSpeed*FPSfactor
									If Rand(200)=1 And NetworkServer\MainPlayer Then MoveEntity n\Collider, 0, 0, 0.5
									RotateEntity n\Collider, 0, EntityYaw(n\Collider), 0, True
								Else
									For i = 0 To 4
										If n\Path[0]\connected[i] <> Null Then
											If EntityDistance(n\NearObject, n\Path[0]\connected[i]\obj) > dist Then
			...
			Case NPCtype372 ;------------------------------------------------------------------------------------------------------------------
				;[Block]
				RN$ = Player[n\NearID]\PlayerRoomName
				If RN$ <> "pocketdimension" And RN$ <> "dimension1499" Then 
					If n\Idle Then
						HideEntity(n\obj)
						If Rand(50) = 1 And (BlinkTimer < -5 And BlinkTimer > -15) Then
							ShowEntity(n\obj)
							angle# = EntityYaw(n\NearObject)+Rnd(-90,90)
							
							dist = Rnd(1.5, 2.0)
							PositionEntity(n\Collider, EntityX(n\NearObject) + Sin(angle) * dist, EntityY(n\NearObject)+0.2, EntityZ(n\NearObject) + Cos(angle) * dist)
							n\Idle = False
							n\State = Rand(20, 60)
					...
					Else
						...
						If EntityInView(n\obj, Camera) Then
							...
							temp = CreatePivot()
							PositionEntity temp, EntityX(n\NearObject), EntityY(n\NearObject), EntityZ(n\NearObject)
							PointEntity temp, n\Collider
							
							angle =  WrapAngle(EntityYaw(n\NearObject)-EntityYaw(temp))
							If angle < 180 Then
								RotateEntity n\Collider, 0, EntityYaw(n\NearObject)-80, 0		
							Else
								RotateEntity n\Collider, 0, EntityYaw(n\NearObject)+80, 0
							EndIf
							FreeEntity temp
				...
				Select n\State
					Case 0,1
						...
						If n\State=1 And (Not NoTarget) Then
							If Abs(EntityX(n\NearObject)-EntityX(n\Collider))< 30.0 Then
								If Abs(EntityZ(n\NearObject)-EntityZ(n\Collider))<30.0 Then
									If Abs(EntityY(n\NearObject)-EntityY(n\Collider))<20.0 Then
										If Rand(20)=1 Then 
											If EntityVisible(n\NearObject, n\Collider) Then
												n\State = 2
						...
						If Abs(EntityX(target)-EntityX(n\Collider)) < 55.0 Then
							If Abs(EntityZ(target)-EntityZ(n\Collider)) < 55.0 Then
								If Abs(EntityY(target)-EntityY(n\Collider))< 20.0 Then
									...
									;If Distance(EntityX(n\NearObject),EntityZ(n\NearObject),EntityX(n\Collider),EntityZ(n\Collider)) > 6.5 Then
				...
				If dist < HideDistance
					...
					Select n\State 
						...
						Case 1 ;idle
							...
							If dist < 1.8 Then 
								If Abs(DeltaYaw(n\Collider, n\NearObject))<20 Then 
									n\State = 2
										...
										If dist < 1.8 Then
											If Abs(DeltaYaw(n\Collider, n\NearObject))<20 Then 
												If WearingHazmat Then
													...
													multiplayer_WriteTempSound("SFX\General\BodyFall.ogg", EntityX(Collider), EntityY(Collider), EntityZ(Collider), 5.0)
												Else
													BlurTimer = 100
													Injuries = Injuries+Rnd(1.0,1.5)
													PlaySound_Strict DamageSFX(Rand(2,3))
													multiplayer_WriteSound(DamageSFX(Rand(2,3)), EntityX(Collider), EntityY(Collider), EntityZ(Collider), 5.0)
			...
			Case NPCtype860
				;[Block]
				If Player[n\NearID]\PlayerRoomName = "room860" Then
					Local fr.Forest=Room[Player[n\NearID]\PlayerRoomID]\fr;Object.Forest(e\room\Objects[1])
					...
					Select n\State
						...
						Case 1 ;appears briefly behind the trees
							...
							If EntityY(n\Collider)<= -100 Then
								...
								TFormPoint(EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject),0,fr\Forest_Pivot)
							...
							Else
								...
								Else
									...
									Else
										angle = CurveAngle(point_direction(EntityX(n\Collider),EntityZ(n\Collider),EntityX(n\NearObject),EntityZ(n\NearObject)),EntityYaw(n\Collider)+90,20.0)
							...
							If (dist<1.1 Or (n\Frame>451 And n\Frame<493) Or KillTimer < 0) Then
								if n\NearID = NetworkServer\MyID Then
									...
									If (prevFrame < 461 And n\Frame=>461) Then 
										If KillTimer => 0 Then Kill("was killed by SCP-860") : KillAnim = 0
										PlaySound_Strict(n\Sound)
									EndIf
				...
				If n\State < 66 Then 
					Select n\State
						...
						Case 2
							Local rom.Rooms
							For r.Rooms = Each rooms
								if r\RoomTemplate\Name = "room3storage" Then rom = r : Exit
							Next
							n\State2 = Max(n\State2, (n\PrevState-3))
							
							dist = EntityDistance(n\Collider, rom\Objects[n\State2])
							...
							If (prevFrame<664 And n\Frame=>664) Or (prevFrame>673 And n\Frame<654) Then
								...
								EndIf
							EndIf
							PointEntity n\obj, rom\Objects[n\State2]
							RotateEntity n\Collider, 0, CurveAngle(EntityYaw(n\obj),EntityYaw(n\Collider),20.0), 0
						...
						Case 3
							If EntityVisible(n\NearObject, n\Collider) Then
								...
								n\EnemyX = EntityX(n\NearObject)
								n\EnemyZ = EntityZ(n\NearObject)
								n\LastSeen = 10*7
							EndIf
							...
							If n\LastSeen > 0 And (Not NoTarget) Then
								...
								If (n\Frame=>18.0 And n\Frame<68.0) Then
									...
									If temp Then
										If Distance(EntityX(n\NearObject), EntityZ(n\NearObject), EntityX(n\Collider), EntityZ(n\Collider))<1.5 Then
											if n\nearID = NetworkServer\MyID Then
												PlaySound_Strict n\Sound2
												multiplayer_WriteTempSound("SFX\General\Slash1.ogg", EntityX(Collider), EntityY(Collider), EntityZ(Collider), 5.0)
												Injuries = Injuries + Rnd(1.5, 2.5)-WearingVest*0.5
												BlurTimer = 500
											Else
												GiveDamage(n\NearID, Rnd(1.5,2.5))
											EndIf
										Else
											n\Frame	 = 449
									...
									If Injuries>4.0 And n\NearID = NetworkServer\MyID Then 
								...
								Else
									If n\LastSeen = 10*7 Then 
										...
										If Distance(EntityX(n\NearObject), EntityZ(n\NearObject), EntityX(n\Collider), EntityZ(n\Collider))<1.1 Then ;player is visible
											n\Frame = 18
								...
								angle = VectorYaw(EntityX(n\NearObject)-EntityX(n\Collider), 0.0, n\EnemyZ-EntityZ(n\Collider))
					...
					If n\State < 3 And (Not NoTarget) And (Not n\IgnorePlayer) Then
						...
						If dist < 4.0 Then dist = dist - EntityVisible(n\NearObject, n\Collider)
						If Player[n\NearID]\PlayerSoundVolume*1.2>dist Or dist < 1.5 Then
							If n\State3 = 0 Then
								If n\Sound <> 0 Then FreeSound_Strict n\Sound : n\Sound = 0
								n\Sound = LoadSound_Strict("SFX\SCP\939\"+(n\ID Mod 3)+"Attack"+Rand(1,3)+".ogg")
								n\SoundChn = PlaySound2(n\Sound, Camera, n\Collider)										
								
								if n\NearID = NetworkServer\MyID Then PlaySound_Strict(LoadTempSound("SFX\SCP\939\attack.ogg"))
								n\State3 = 1
							EndIf
							
							n\State = 3
						ElseIf Player[n\NearID]\PlayerSoundVolume*1.6>dist
			...
			Case NPCtype066
				;[Block]
				dist = Distance(EntityX(n\NearObject),EntityZ(n\NearObject),EntityX(n\Collider),EntityZ(n\Collider))
				...
				Select n\State
					...
					Case 1 ;staring at the player
						...
						If n\Frame<451 Then
							angle = WrapAngle(CurveAngle(DeltaYaw(n\Collider, n\NearObject)-180, (AnimTime(n\obj)-2.0)/1.2445, 15.0))
							;0->360 = 2->450
								...
								If n\State2 = 0 Then
									...
									Select Rand(1,6)
										...
										Case 3
											...
											Next
										Case 4
											If PlayerRoom\RoomTemplate\DisableDecals = False And n\NearID = NetworkServer\MyID Then
												CameraShake = 5.0
												...
												If Distance(EntityX(n\NearObject),EntityZ(n\NearObject),EntityX(n\Collider),EntityZ(n\Collider))<0.8 Then
...
;							EndIf
							...
							angle = VectorYaw(EntityX(n\NearObject)-EntityX(n\Collider),0,EntityZ(n\NearObject)-EntityZ(n\Collider))
							RotateEntity n\Collider,0.0,CurveAngle(angle,EntityYaw(n\Collider),20.0),0.0
							
							If n\State3<900 And n\NearID = NetworkServer\myID Then
...
;							EndIf
							...
							If (n\Frame>271.0 And prevFrame<=271.0) Or (n\Frame>301.0 And prevFrame<=301.0) Or (n\Frame>314.0 And prevFrame<=314.0) Then
								if not ChannelPlaying(n\channelmultiplayer) Then n\channelmultiplayer = PlaySound2(LoadTempSound("SFX\SCP\966\Idle"+Rand(1,3)+".ogg"), Camera, n\Collider)
							EndIf
						...
						Case 5,6,8 ;walking or chasing
							...
							Else
								...
								If n\State = 8 And dist<32 Then
									...
									If (Not EntityVisible(n\Collider,n\NearObject)) Then
										If n\PathStatus = 2 Then
											n\CurrSpeed = 0
											;SetNPCFrame(n,201)
											SetNPCFrame(n,2)
										...
										EndIf
									Else
										n\Angle = VectorYaw(EntityX(n\NearObject)-EntityX(n\Collider),0,EntityZ(n\NearObject)-EntityZ(n\Collider))
										n\CurrSpeed = CurveValue(n\Speed,n\CurrSpeed,10.0)
...
;								EndIf
							...
							If dist<1.0 Then
								...
								If n\Frame>470.0 And prevFrame<=470.0 Or n\Frame>500.0 And prevFrame<=500.0 Or n\Frame>527.0 And prevFrame<=527.0
									if n\NearID = NetworkServer\MyID Then
										...
										multiplayer_WriteTempSound("SFX\General\Slash"+Rand(1,2)+".ogg", EntityX(n\Collider), EntityY(n\Collider), EntityZ(n\Collider))
									
										Injuries = Injuries + Rnd(0.5,1.0)
									Else
										GiveDamage(n\NearID, Rnd(0.5, 1.0))
									EndIf
								EndIf	
							EndIf
				...
				Else
					...
					EndIf
				EndIf
				If WearingNightVision=0 Then HideEntity n\obj
			...
			Case NPCtype1499
				...
				If (Not n\Idle) And EntityDistance(n\Collider,n\NearObject)<HideDistance*3 Then
						...
						Case 3 ;slashing at the player
							...
							If n\State2 = 1
								...
								If prevFrame < 89 And n\Frame=>89
									If dist > 0.85 Or Abs(DeltaYaw(n\Collider,n\NearObject))>60.0
										;Miss
									ElseIf n\NearID = NetworkServer\MyID
										Injuries = Injuries + Rnd(0.75,1.5)
										PlaySound2(LoadTempSound("SFX\General\Slash"+Rand(1,2)+".ogg"), Camera, n\Collider)
										multiplayer_WriteTempSound("SFX\General\Slash"+Rand(1,2)+".ogg")
										
										If Injuries > 10.0
											Kill("was killed by SCP-1499-2")
										...
										EndIf
									Else
										GiveDamage(n\NearID, rnd(0.75,1.5))
									EndIf
								ElseIf n\Frame => 99
									n\State2 = 0.0
									n\State = 1
							...
							Else
								...
								If prevFrame < 189 And n\Frame=>189
									If dist > 0.85 Or Abs(DeltaYaw(n\Collider,n\NearObject))>60.0
										;Miss
									Else If n\NearID = NetworkServer\MyID
										Injuries = Injuries + Rnd(0.75,1.5)
										PlaySound2(LoadTempSound("SFX\General\Slash"+Rand(1,2)+".ogg"), Camera, n\Collider)
										multiplayer_WriteSound("SFX\General\Slash"+Rand(1,2)+".ogg", EntityX(Collider), EntityY(Collider), EntityZ(Collider))
			...
			Case NPCtype008
				...
				If (Not n\IsDead)
					if n\mpframe >= 344 And n\mpframe <= 363 Then n\IsDead = True
					...
					Select n\State
						...
						Case 2 ;Being active
							...
							If PlayerSeeAble=1 Or n\State2 > 0.0
								...
								If EntityDistance(n\Collider,n\NearObject)<1.0
									If (Abs(DeltaYaw(n\Collider,n\NearObject))<=60.0)
										n\State = 3
									EndIf
								EndIf
						...
						Case 3 ;Attacking
							...
							If (n\Frame => 146 And prevFrame < 146)
								If EntityDistance(n\Collider,n\NearObject)<1.1
									If (Abs(DeltaYaw(n\Collider,n\NearObject))<=60.0) And n\NearID = NetworkServer\MyID Then
										PlaySound_Strict DamageSFX(Rand(5,8))
										multiplayer_WriteSound(DamageSFX(Rand(5,8)), EntityX(Collider), EntityY(Collider), EntityZ(Collider), 5.0)
										Injuries = Injuries+Rnd(0.4,1.0)
						...
						Case 4 ;Idling
							...
							Else
								If OtherPlayerInReachableRoom(n\nearid) ;Player is in a room where SCP-008-1 can teleport to
									If Rand(50-(20*SelectedDifficulty\aggressiveNPCs))=1
										ShowEntity n\Collider
										ShowEntity n\obj
										For w.waypoints = Each WayPoints
											If w\door=Null And w\room\dist < HideDistance And Rand(3)=1 Then
												If EntityDistance(w\room\obj,n\Collider)<EntityDistance(n\NearObject,n\Collider)
		...
		if NetworkServer\MainPlayer = False Then n\DropSpeed = 0
		Local gravityDist# = Distance(EntityX(n\NearObject),EntityZ(n\NearObject),EntityX(n\Collider),EntityZ(n\Collider))
		
		If gravityDist<HideDistance*0.7 Or n\NPCtype = NPCtype1499 Then
			;if LinePick(EntityX(n\obj),EntityY(n\obj),EntityZ(n\obj),0,-10,0) <> 0 Then
			if CheckForMultiPlayerInFacility(n\NearObject) = n\InFacility Then
				...
				Else
					If ShouldEntitiesFall
						UpdateGravity% = True
						if n\NPCtype = NPCtype860 Then
							if room860event <> Null Then UpdateGravity = PlayerInRoom(room860event)
						EndIf
						if UpdateGravity Then n\DropSpeed# = Max(n\DropSpeed - 0.005*FPSfactor*n\GravityMult,-n\MaxGravity)
					Else
						n\DropSpeed# = 0.0
					EndIf
				EndIf
		...
		if Not NetworkServer\MainPlayer Then
			if Distance3(n\x,n\y,n\z, n\smoothx,n\smoothy,n\smoothz) > 4 Then
				n\smoothx = n\x
				n\smoothy = n\y
				n\smoothz = n\z
			EndIf
			n\smoothx = CurveValue(n\x, n\smoothx, 5)
			n\smoothy = CurveValue(n\y, n\smoothy, 5)
			n\smoothz = CurveValue(n\z, n\smoothz, 5)
			;n\smoothframe = CurveValue(n\mpFrame, n\smoothframe, 5)
			n\smoothyaw = CurveAngle(n\yaw, n\smoothyaw, 5)
			
			SetAnimTime(n\obj, n\mpFrame)
			PositionEntity(n\Collider,n\smoothx, n\smoothy,n\smoothz, True)
			RotateEntity(n\Collider,EntityPitch(n\Collider, True),n\smoothyaw,EntityRoll(n\Collider, True), True)
			ResetEntity(n\Collider)
		EndIf
		if n\NPCtype <> NPCtype966 And n\NPCtype <> NPCtype5131 And n\NPCtype <> NPCtype372 Then
			if gravityDist < HideDistance*4 Then
				ShowEntity n\obj
		...
		If w\door = Null Then
			...
			If xtemp < 10.0 And xtemp > 1.0 Then 
				...
				If ztemp < 10.0 And ztemp > 1.0 Then
					If (EntityDistance(n\NearObject, w\obj)>16-(8*SelectedDifficulty\aggressiveNPCs)) Then
						;teleports to the nearby waypoint that takes it closest to the player
						Local newDist# = EntityDistance(n\NearObject, w\obj)
...
Function MeNPCSeesPlayer%(me.NPCs,disablesoundoncrouch%=False)
	...
	If (Not PlayerDetected) Or me\NPCtype <> NPCtypeMTF
		...
		If EntityDistance(me\NearObject,me\Collider)>(8.0-Player[me\NearID]\CrouchState+Player[me\NearID]\PlayerSoundVolume) Then Return False
		
		;spots the player if he's either in view or making a loud sound
		If Player[me\NearID]\PlayerSoundVolume>1.0
			If (Abs(DeltaYaw(me\Collider,me\NearObject))>60.0) And EntityVisible(me\Collider,me\NearObject)
				Return 1
			ElseIf (Not EntityVisible(me\Collider,me\NearObject))
			...
			EndIf
		Else
			If (Abs(DeltaYaw(me\Collider,me\NearObject))>60.0) Then Return False
		EndIf
		Return EntityVisible(me\Collider,me\NearObject)
	Else
		If EntityDistance(me\NearObject,me\Collider)>(8.0-Player[me\NearID]\CrouchState+Player[me\NearID]\PlayerSoundVolume) Then Return 3
		If EntityVisible(me\Collider, me\NearObject) Then Return True
		
		;spots the player if he's either in view or making a loud sound
		If Player[me\NearID]\PlayerSoundVolume>1.0 Then Return 2
		Return 3
	EndIf
		...
		Select Int(n\State) ;what is this MTF doing
			Case 0 ;wandering around
                ...
                If temp>False Then
					...
					;If EntityDistance(n\Collider,n\NearObject)>HideDistance*0.7
					;	TeleportMTFGroup(n)
					;EndIf
                EndIf
			...
			Case 1 ;searching for player
                ...
                If MeNPCSeesPlayer(n) = True Then
					...
					If playerDist < 4.0 Then
						...
						Local angle# = VectorYaw(EntityX(n\NearObject)-EntityX(n\Collider),0,EntityZ(n\NearObject)-EntityZ(n\Collider))
						...
						If n\Reload =< 0 And KillTimer = 0 Then
							If EntityVisible(n\Collider, Camera) Then
								...
								If angle < 5 Or angle > 355 Then 
									...
									Shoot(n,EntityX(pvt),EntityY(pvt),EntityZ(pvt),5.0/playerDist, False)
									n\Reload = 7
							...
							If n2\NPCtype = NPCtypeMTF And n2 <> n
								If n2\State = 0
									If EntityDistance(n\Collider,n2\Collider)<6.0
										...
										n2\State = 1
										n2\EnemyX = EntityX(n\NearObject,True)
										n2\EnemyY = EntityY(n\NearObject,True)
										n2\EnemyZ = EntityZ(n\NearObject,True)
										n2\State2 = n\State2
										n2\PathTimer=0.0
										n2\PathStatus=0
			...
			Case 6 ;seeing the player as a 049-2 instance
				...
				If n\Reload =< 0 And KillTimer = 0 Then
					If EntityVisible(n\Collider, n\NearObject) Then
						;angle# = WrapAngle(angle - EntityYaw(n\Collider))
						;If angle < 5 Or angle > 355 Then
						If (Abs(DeltaYaw(n\Collider,n\NearObject))<50.0)
							;prev% = KillTimer
							...
							Shoot(n,EntityX(pvt),EntityY(pvt),EntityZ(pvt),0.9, False)
							n\Reload = 7
...
Function Shoot(n.NPCs, x#, y#, z#, hitProb# = 1.0, particles% = True, instaKill% = False)
	...
	If instaKill Then
		if n\NearID = NetworkServer\MyID Then
			If (Not GodMode) Then
				Kill("was killed by shoot") 
				PlaySound_Strict BullethitSFX
				multiplayer_WriteSound(BullethitSFX,EntityX(Collider),EntityY(Collider),EntityZ(Collider), 5.0)
			EndIf
		Else
			GiveDamage(n\NearID, -1)
		EndIf
	...
	If Rnd(1.0) =< hitProb Then
		If n\NearID = NetworkServer\MyID And (Not GodMode) Then
			...
			If WearingVest>0 Then
				...
				If Injuries >= 5
					If Rand(3) = 1 Then Kill("was killed by shoot")
				EndIf
			Else
				Select Rand(6)
					Case 1
			...
			multiplayer_WriteSound(BullethitSFX, EntityX(Collider), EntityY(Collider), EntityZ(Collider), 5.0)
		Else
			GiveDamage(n\NearID, Rnd(2, 4))
		EndIf
	ElseIf particles And ParticleAmount>0
		
		pvt = CreatePivot()
		PositionEntity pvt, EntityX(n\NearObject),(EntityY(n\NearObject)+EntityY(Camera))/2,EntityZ(n\NearObject)
...
Function Find860Angle(n.NPCs, fr.Forest)
	...
	If xt<>playerx Or zt<>playerz Then ;the monster is not on the same tile as the player
		...
		Next
	Else
		Return point_direction(EntityX(n\Collider),EntityZ(n\Collider),EntityX(n\NearObject),EntityZ(n\NearObject))+180
	EndIf		
End Function
Function Console_SpawnNPC(c_input$, c_state$ = "", playerid = 0)
	Local n.NPCs
	Local consoleMSG$
	Local Ent = Collider
	
	if playerid > 0 Then
		if Player[playerid] <> Null Then Ent = Player[playerid]\Pivot
	EndIf
	Select c_input$ 
		Case "008", "008zombie"
			n.NPCs = CreateNPC(NPCtype008, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
			n\State = 1
			consoleMSG = "SCP-008 infected human spawned."
			
		Case "049", "scp049", "scp-049"
			n.NPCs = CreateNPC(NPCtype049, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
			n\State = 1
			consoleMSG = "SCP-049 spawned."
			
		Case "049-2", "0492", "scp-049-2", "scp049-2", "049zombie"
			n.NPCs = CreateNPC(NPCtypeZombie, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
			n\State = 1
			consoleMSG = "SCP-049-2 spawned."
			
		Case "066", "scp066", "scp-066"
			n.NPCs = CreateNPC(NPCtype066, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
			consoleMSG = "SCP-066 spawned."
			
		Case "096", "scp096", "scp-096"
			n.NPCs = CreateNPC(NPCtype096, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
			n\State = 5
			If (Curr096 = Null) Then Curr096 = n
			consoleMSG = "SCP-096 spawned."
			
		Case "106", "scp106", "scp-106", "larry"
			n.NPCs = CreateNPC(NPCtypeOldMan, EntityX(Ent), EntityY(Ent) - 0.5, EntityZ(Ent))
			n\State = -1
			consoleMSG = "SCP-106 spawned."
			
		Case "173", "scp173", "scp-173", "statue"
			n.NPCs = CreateNPC(NPCtype173, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
			Curr173 = n
			If (Curr173\Idle = 3) Then Curr173\Idle = False
			consoleMSG = "SCP-173 spawned."
		Case "372", "scp372", "scp-372"
			n.NPCs = CreateNPC(NPCtype372, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
			consoleMSG = "SCP-372 spawned."
			
		Case "513-1", "5131", "scp513-1", "scp-513-1"
			n.NPCs = CreateNPC(NPCtype5131, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
		...
		Case "966", "scp966", "scp-966"
			n.NPCs = CreateNPC(NPCtype966, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
		...
		Case "1499-1", "14991", "scp-1499-1", "scp1499-1"
			n.NPCs = CreateNPC(NPCtype1499, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
			consoleMSG = "SCP-1499-1 instance spawned."
			
		Case "class-d", "classd", "d"
			n.NPCs = CreateNPC(NPCtypeD, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
			consoleMSG = "D-Class spawned."
			
		Case "guard"
			n.NPCs = CreateNPC(NPCtypeGuard, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
			consoleMSG = "Guard spawned."
			
		Case "mtf"
			n.NPCs = CreateNPC(NPCtypeMTF, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
			consoleMSG = "MTF unit spawned."
			
		Case "apache", "helicopter"
			n.NPCs = CreateNPC(NPCtypeApache, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
			consoleMSG = "Apache spawned."
			
		Case "tentacle"
			n.NPCs = CreateNPC(NPCtypeTentacle, EntityX(Ent), EntityY(Ent), EntityZ(Ent))
			consoleMSG = "SCP-035 tentacle spawned."
			
		Case "clerk"
			n.NPCs = CreateNPC(NPCtypeClerk, EntityX(Ent), EntityY(Ent) + 0.2, EntityZ(Ent))
...
End Function
Function ManipulateNPCBones(n.NPCs) ; want to optimize
	Local bone%,pvt%,bonename$
	Local maxvalue#,minvalue#,offset#,smooth#
	Local i%
	Local tovalue#
	;n\npcboneManipulate[0] = GetNPCManipulationValue(n\NPCNameInSection,n\BoneToManipulate,"bonename",0)
	;n\npcboneManipulate[1] = GetNPCManipulationValue(n\NPCNameInSection,n\BoneToManipulate,"controller_max",1)
	;For i = 2 To n\npcboneManipulate[1]
	;	GetNPCManipulationValue(n\NPCNameInSection,n\BoneToManipulate,"controlleraxis"+i,0)
	;	if GetNPCManipulationValue(n\NPCNameInSection,n\BoneToManipulate,"controlleraxis"+i,0) = "pitch"
	;		
	;	Elseif GetNPCManipulationValue(n\NPCNameInSection,n\BoneToManipulate,"controlleraxis1"+i,0) = "yaw"
	;Next
	If n\ManipulateBone
					...
					For i = 1 To GetNPCManipulationValue(n\NPCNameInSection,n\BoneToManipulate,"controller_max",1)
						If GetNPCManipulationValue(n\NPCNameInSection,n\BoneToManipulate,"controlleraxis"+i,0) = "pitch"
							...
							If GetNPCManipulationValue(n\NPCNameInSection,n\BoneToManipulate,"controlleraxis"+i+"_inverse",3)
								tovalue = -DeltaPitch(bone,GetPlayerHead(n\NearID))+offset
							Else
								tovalue = DeltaPitch(bone,GetPlayerHead(n\NearID))+offset
							EndIf
						...
						ElseIf GetNPCManipulationValue(n\NPCNameInSection,n\BoneToManipulate,"controlleraxis1",0) = "yaw"
							...
							offset# = GetNPCManipulationValue(n\NPCNameInSection,n\BoneToManipulate,"controlleraxis"+i+"_offset",2)
							If GetNPCManipulationValue(n\NPCNameInSection,n\BoneToManipulate,"controlleraxis"+i+"_inverse",3)
								tovalue = -DeltaYaw(bone,GetPlayerHead(n\NearID))+offset
							Else
								tovalue = DeltaYaw(bone,GetPlayerHead(n\NearID))+offset
							EndIf
...
Function OtherPlayerInReachableRoom(id, canSpawnIn049Chamber%=False)
	Local RN$ = Player[id]\PlayerRoomName
	Local e.Events, temp
	...
	EndIf
	;Player is at GateB and is at the surface, returning false
	If RN = "exit1" And EntityY(Player[id]\pivot)>1040.0*RoomScale Then
		Return False
	EndIf
	;Player is in 860's test room and inside the forest, returning false
	temp = False
	...
	If (Not canSpawnIn049Chamber) Then
		If SelectedDifficulty\aggressiveNPCs = False Then
			If RN = "room049" And EntityY(Player[id]\pivot)<=-2848*RoomScale Then
				Return False
			EndIf
		EndIf
	EndIf
...
;~F#0#89D
;~B#196#12A8#1342#13DB#158F#169A#185B#18B7
;~C#Blitz3D