...
AppTitle "SCP - Containment Breach Multiplayer Mod"
...
Function ExecuteConsoleCommand(cinput$, ErrorMessage% = True, SendConsoleTick% = True)
	...
	if SendConsoleTick Then
		if udp_GetStream() Then
			udp_WriteByte M_GLOBALCONSOLE
			udp_WriteByte NetworkServer\MyID
			udp_WriteLine cinput
			udp_SendMessage()
		EndIf
	EndIf
	...
	Select Lower(StrTemp)
		...
		Case "kill","suicide"
			...
			End Select
			Kill("decided to gnaw his veins")
			;[End Block]
		Case "playmusic"
			;[Block]
	...
	Else
		...
		if (not MenuOpen) or udp_GetStream() Then
			...
			While (ft\accumulator>0.0)
				...
				if SelectedDoor = Null And Spectate\SpectatePlayer = -1 Then
					If CanInteract() Then
						MouseLook(true)
...
Function Kill(killmsg$ = "was killed", ForceKill = False)
	...
	If KillTimer >= 0 Then
		...
		br.BreachTypes = GetBreachType(MyPlayer\BreachType)
		if GetSecondPackedValue(br\DeadAnimationData) <> 0 And (Not NetworkServer\Breach) Then CreateDeadBody(MyPlayer\x,MyPlayer\y-br\FixPivot,MyPlayer\z,MyPlayer\yaw+br\FixRotate, br\model, br\texture, GetFirstPackedValue(br\DeadAnimationData),GetSecondPackedValue(br\DeadAnimationData), br\scale, br\FixPitch)
				...
				If temp < 180 And (Shake Mod 360) >= 180 And KillTimer>=0 Then
					If CurrStepSFX=0 Then
						...
						Else
							...
							ChannelVolume tempchn, (1.0-(Crouch*0.6))*SFXVolume#
						End If
						multiplayer_WriteSound(StepSFX(temp,0, Rand(0,7)), EntityX(Collider), EntityY(Collider), EntityZ(Collider), 8.0)
					ElseIf CurrStepSFX=1
						tempchn% = PlaySound_Strict(Step2SFX(Rand(0, 2)))
						ChannelVolume tempchn, (1.0-(Crouch*0.4))*SFXVolume#
						multiplayer_WriteSound(Step2SFX(Rand(0, 2)), EntityX(Collider), EntityY(Collider), EntityZ(Collider), 8.0)
					ElseIf CurrStepSFX=2
						tempchn% = PlaySound_Strict(Step2SFX(Rand(3,5)))
						ChannelVolume tempchn, (1.0-(Crouch*0.4))*SFXVolume#
						multiplayer_WriteSound(Step2SFX(Rand(3,5)), EntityX(Collider), EntityY(Collider), EntityZ(Collider), 8.0)
					ElseIf CurrStepSFX=3
						If Sprint = 2.5 Then
							PlayerSoundVolume = Max(4.0,PlayerSoundVolume)
							tempchn% = PlaySound_Strict(StepSFX(0, 0, Rand(0, 7)))
							ChannelVolume tempchn, (1.0-(Crouch*0.6))*SFXVolume#
							multiplayer_WriteSound(StepSFX(0, 0, Rand(0, 7)), EntityX(Collider), EntityY(Collider), EntityZ(Collider), 8.0)
						Else
							PlayerSoundVolume = Max(2.5-(Crouch*0.6),PlayerSoundVolume)
							tempchn% = PlaySound_Strict(StepSFX(0, 1, Rand(0, 7)))
							ChannelVolume tempchn, (1.0-(Crouch*0.6))*SFXVolume#
							multiplayer_WriteSound(StepSFX(0, 1, Rand(0, 7)), EntityX(Collider), EntityY(Collider), EntityZ(Collider), 8.0)
						End If
					EndIf
		...
		ElseIf AchievementsMenu <= 0 And OptionsMenu <= 0 And QuitMSG <= 0 And KillTimer >= 0 And SuicideMsg > 0 
			If DrawButton(x, y + 60*MenuScale, 390*MenuScale, 60*MenuScale, "Suicide") Then
				if MyPlayer\BreachType <> MODEL_WAIT Then 
					Kill("decided to gnaw his veins", True)
					MenuOpen = False
					MENU_OPEN_TYPE = MenuOpen
