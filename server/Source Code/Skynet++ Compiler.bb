...
Global MainFunction% = False
Global MainFunctionError% = False
Global MainFunctionErrorBlock% = False
;Error output
Global SE_ERROR
Global SE_ERROR_MESSAGE$			; Something wrong
Global SE_ERROR_DUMP_LINE=True

Function SE_SetError(Message$, clearcompiler=true)
	SE_ERROR=clearcompiler
	SE_ERROR_MESSAGE=Message
	if clearcompiler then SE_ClearCompiler()
End Function

Function SE_Error(Token.SE_Token, Message$, ClearCompiler%=true)
	If SE_ERROR_DUMP_LINE Then Message=Message+Chr(13)+Chr(13)+Token\TokenLine\LineText
	SE_SetError("error at line "+Token\TokenLine\Number+", column "+Token\Offset+": "+Message, ClearCompiler)
End Function
...
Function SE_PP_Include(FileName$, SrcFileName$)
	...
	For I=Length To 1 Step -1
		If Mid(SrcFileName, I, 1)="\" Or Mid(SrcFileName, I, 1)="/"
...
Function SE_ParseLines(FileName$)
	...
	If File=0
		SE_Warn(Null, "unable to open file "+Chr(34)+FileName+Chr(34))
		Return
	EndIf
	SeekFile File, 0
	While Not Eof(File)

		LineText$=Trim(ReadLine(File))
		Length=Len(LineText)
		Offset=1
		RemBlockOpened = False
		RemOffset=0
		SkipLine=False
		...
		If Left(LineText, 1)="#"
				...
				Select PP_Token\Token
					Case "playerscript","PLAYERSCRIPT"
						MainFunctionErrorBlock% = True
					Case "define"
						msg$ = ""
						msg = DeleteStringFromString(LineText, "#define ")
						msg = Replace(msg, " = ", "=")
						msg = Replace(msg, " =", "=")
						msg = Replace(msg, "= ", "=")
						
						if Instr(msg, "=") Then 
							replacetoken$ = Right(msg, Len(msg)-InstrLatest(msg, "="))
							changetoken$ = Left(msg, InstrLatest(msg, "=")-1)
							For def.defines = Each defines
								replacetoken = Replace(replacetoken, def\change, def\rep)
							Next
							DeclareDefine(changetoken,replacetoken)
						Elseif Instr(msg, " ") Then
							replacetoken$ = Right(msg, Len(msg)-InstrLatest(msg, " "))
							changetoken$ = Left(msg, InstrLatest(msg, " ")-1)
							For def.defines = Each defines
								replacetoken = Replace(replacetoken, def\change, def\rep)
					...
					Case "define_ex"
						msg$ = ""
						msg = DeleteStringFromString(LineText, "#define_ex ")
						msg = Replace(msg, " = ", "=")
						msg = Replace(msg, " =", "=")
						msg = Replace(msg, "= ", "=")
						
						if Instr(msg, "=") Then 
							replacetoken$ = Right(msg, Len(msg)-Instr(msg, "="))
							changetoken$ = Left(msg, Instr(msg, "=")-1)
							For def.defines = Each defines
								replacetoken = Replace(replacetoken, def\change, def\rep)
							Next
							DeclareDefine(changetoken,replacetoken)
					...
					Default:
						SE_Warn(Null, "undefined preprocessor directive execution ("+PP_Token\Token+")")
				End Select
		...
		Else
			For def.defines = Each defines
				LineText = Replace(LineText, def\change, def\rep)
			Next
		EndIf
	...
	If Parent\Token<>Null
		If Parent\Token\TokenType=SE_TOKEN_IDENTIFIER
			...
			If FuncPtr<>Null
				...
				if MainFunction And (not MainFunctionError) and (not MainFunctionErrorBlock) Then 
					SE_Warn(Parent\Token, "execute functions in main block won't work")
					;MainFunctionError = True
				EndIf
			Else
				...
				if Not SE_IsValidFunction(Lower(ValueA\Value)) Then SE_Warn(Parent\Token, "undefined function")
...
Function SE_Compile(FileName$)
	...
	While Block<>Null
		If Block\BlockType<>SE_BLOCK_DEF Then
			MainFunction = True
			SE_CompileBlock(Block)
			MainFunction = False
		EndIf
	...
	if MainFunctionError Then
		SE_ERROR = True
		MainFunctionError = False
		Return
	EndIf
	
	MainFunctionError = False
	MainFunctionErrorBlock = False
