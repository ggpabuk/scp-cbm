...
Function InitEvents()
	Local e.Events
	If Server\IntroEnabled And (Not Server\Breach) Then CreateEvent("173", "173", 0)
	If (Not Server\Breach) Then CreateEvent("alarm", "start", 0)
...
Function FindEventObject(e.Events)
	If e\Tick >= Server\PlayersCount Or e\NearObjectQueue+1 = Server\Maxplayers+1 Then 
		e\NearObjectQueue = 0
		e\Tick = 0
	EndIf
	For i = e\NearObjectQueue+1 To Server\Maxplayers
...
Function UpdateBreachEvents()
	...
	For e.Events = Each Events
		...
		CurrentPlayerInEvent = e\NearObjectQueue
		Select e\EventConst
			Case e_room012
				;[Block]
					...
					Else
						If 0 Then
							...
							Else
								...
								EndIf
							EndIf
							If 0;Server\FullSync Then
								For p2.players = Each players
									If p2\playerroomid = e\room\ID Then
										If ReadBool(p2\wearingdir, 4)=False Then
											;temp = False
											If EntityVisible(e\room\Objects[2],p2\Hitbox) Then 							
											...
											;DebugLog "WHERE IS IT?!"	
												...
												RotateEntity(p2\pivot, EntityPitch(p2\pivot), CurveAngle(EntityYaw(pvt), EntityYaw(p2\pivot), 80-(e\EventState3/200.0)), 0)
												...
												dist = Distance(EntityX(p2\pivot),EntityZ(p2\pivot),EntityX(e\room\Objects[2],True),EntityZ(e\room\Objects[2],True))
												...
												If dist < 0.6 Then
													...
													ElseIf e\EventState3>13*70 And e\EventState3-FPSfactor=<13*70
														...
														MsgTimer = 7*70
														p2\Injuries=p2\Injuries+0.5
														PlaySound_Strict LoadTempSound("SFX\SCP\012\Speech2.ogg")
														;Sound("SFX\SCP\012\Speech2.ogg")
													...
													ElseIf e\EventState3>74*70 And e\EventState3-FPSfactor=<74*70
														...
														MsgTimer = 7*70
														p2\Injuries=p2\Injuries+0.8
														PlaySound_Strict LoadTempSound("SFX\SCP\012\Speech7.ogg")
														Crouch = True
														;Sound("SFX\SCP\012\Speech7.ogg")
														de.Decals = CreateDecal(17,  EntityX(p2\pivot), -768*RoomScale+0.01, EntityZ(p2\pivot),90,Rnd(360),0)
													...
													ElseIf e\EventState3>85*70 And e\EventState3-FPSfactor=<85*70	
														...
														GivePlayerHealth(p2\ID, -1000, "was killed by SCP-012")
													EndIf
													
													RotateEntity(p2\pivot, EntityPitch(p2\pivot), CurveAngle(EntityYaw(p2\pivot)+Sin(e\EventState3*(e\EventState3/2000))*(e\EventState3/300), EntityYaw(p2\pivot), 80), 0)
													
												Else
													angle = WrapAngle(EntityYaw(pvt)-EntityYaw(p2\pivot))
													If angle<40.0 Then
														p2\ForceMove = (40.0-angle)*0.02
													ElseIf angle > 310.0
														p2\ForceMove = (40.0-Abs(360.0-angle))*0.02
													EndIf
												EndIf								
											...
											;	If EntityY(e\NearObject)=-768.0*RoomScale Then
												If (Distance(EntityX(p2\pivot), EntityZ(p2\pivot), EntityX(e\room\RoomDoors[0]\frameobj), EntityZ(e\room\RoomDoors[0]\frameobj))<4.5) And  EntityY(p2\pivot)<-2.5 Then
													pvt% = CreatePivot()
													PositionEntity pvt, EntityX(Camera), EntityY(p2\pivot), EntityZ(Camera)
													PointEntity(pvt, e\room\RoomDoors[0]\frameobj)
													;TurnEntity(pvt, 90, 0, 0)
													user_camera_pitch = CurveAngle(90, user_camera_pitch+90, 100)
													user_camera_pitch=user_camera_pitch-90
													RotateEntity(p2\pivot, EntityPitch(p2\pivot), CurveAngle(EntityYaw(pvt), EntityYaw(p2\pivot), 150), 0)
													
													angle = WrapAngle(EntityYaw(pvt)-EntityYaw(p2\pivot))
													If angle<40.0 Then
														p2\ForceMove = (40.0-angle)*0.008
													ElseIf angle > 310.0
														p2\ForceMove = (40.0-Abs(360.0-angle))*0.008
													EndIf
													FreeEntity pvt	
								...
								Next
							EndIf
							If Instr(DEATHMSG, "Subject D-9341 found in a pool of blood next to") Then e\EventState3 = 0.0
						EndIf
					EndIf
				EndIf
			Case e_room2tunnel
			...
			Case e_room049
				...
				If PlayerInRoom(e) Then
					If EntityY(e\NearObject) <= -2848*RoomScale Then
						...
						e\room\ObjectStatic[7] = False
						e\room\ObjectStatic[9] = False
			...
			Case e_room860
				...
				If fr<>Null Then 
					...
					For p2.players = Each players
						If p2\playerroomid = e\room\ID Then 
							UpdateForest(fr,p2\AnticheatPivot)
							foundplayer = True
						EndIf
					Next
					If Not foundplayer Then HideForest(fr)
				EndIf
				
				If (PlayerInRoom(e) And fr<>Null) Then 
					;the player fell
					If Server\FullSync And Server\Interpolation Then
						For p2.players = Each players
							If p2\playerroomid = e\room\ID Then
								If (Not p2\NoClip)
									If EntityY(p2\AnticheatPivot)>EntityY(fr\Forest_Pivot,True)+0.5
										MoveEntity(p2\AnticheatPivot, 0, ((EntityY(fr\Forest_Pivot,True)+0.5) - EntityY(p2\AnticheatPivot))*FPSfactor, 0)
									EndIf
								EndIf
							EndIf
						Next
				...
				If fr <> Null Then
					If Server\FullSync Then
						For p2.Players = Each Players
							If p2\playerroomid = e\room\ID Then
								If (Abs(Distance(EntityX(e\room\Objects[3],True),EntityZ(e\room\Objects[3],True),EntityX(p2\AnticheatPivot,True),EntityZ(p2\AnticheatPivot,True)))<1.0) Then
									If p2\MouseUpdate > MilliSecs() Then
										PlaySound_Strict(LoadTempSound("SFX\Door\WoodenDoorOpen.ogg"))
										;Sound "SFX\Door\WoodenDoorOpen.ogg"
										...
										pvt = CreatePivot()
										PositionEntity pvt, EntityX(GetCamera(p2\ID), True),EntityY(GetCamera(p2\ID), True),EntityZ(GetCamera(p2\ID), True)
										PointEntity pvt, e\room\obj
										ang# = WrapAngle(EntityYaw(pvt)-EntityYaw(e\room\obj,True))
										If ang > 90 And ang < 270 Then
											PositionEntity p2\Pivot,EntityX(fr\Door[0],True),EntityY(fr\Door[0],True)+EntityY(p2\Pivot,True)+0.5,EntityZ(fr\Door[0],True),True
											RotateEntity p2\Pivot, 0.0, EntityYaw(fr\Door[0],True)-180, 0.0, True
											MoveEntity p2\Pivot, -0.5,0.0,0.5
											p2\ForestYaw = 1.0
										Else
											PositionEntity p2\Pivot,EntityX(fr\Door[1],True),EntityY(fr\Door[1],True)+EntityY(p2\Pivot,True)+0.5,EntityZ(fr\Door[1],True),True
											RotateEntity p2\Pivot, 0.0, EntityYaw(fr\Door[1],True)-180, 0.0, True
											MoveEntity p2\Pivot, -0.5,0.0,0.5
											p2\ForestYaw = 0.0
										EndIf
										FreeEntity pvt
										
										ResetEntity p2\Pivot
										
										SetPlayerPositionEx(p2\ID, p2\PlayerRoomID, EntityX(p2\Pivot), EntityY(p2\Pivot), EntityZ(p2\Pivot))
									EndIf
								Else
									For i = 0 To 1
										If EntityDistance(fr\Door[i], p2\Pivot)<1 Then
											If EntityInView(fr\Door[i], GetCamera(p2\ID)) Then
												DrawHandIcon = True
												If p2\MouseUpdate > MilliSecs() Then
													If i=p2\ForestYaw Then
														BlinkTimer = -10
														...
														PositionEntity p2\Pivot, EntityX(e\room\Objects[2],True),0.5,EntityZ(e\room\Objects[2],True)
														
														RotateEntity p2\Pivot, 0, EntityYaw(e\room\obj,True)+p2\ForestYaw*180, 0
														MoveEntity p2\Pivot, 0,0,1.5
														...
														mp_UpdatePlayerPosition(p2, True)
														SetPlayerPositionEx(p2\ID, p2\PlayerRoomID, EntityX(p2\Pivot), EntityY(p2\Pivot), EntityZ(p2\Pivot))
													Else
														PlaySound_Strict(LoadTempSound("SFX\Door\WoodenDoorBudge.ogg"))
														;Sound "SFX\Door\WoodenDoorBudge.ogg"
			...
			Case e_pocketdimension
				;[Block]
				;the trenches
				Local updatedPD = False
				For p2.players = Each players
					If p2\playerroomid = e\room\ID Then
						If Not updatedPD Then
							...
							If e\EventState = 0 Then e\EventState = 0.1
							...
							e\room\ObjectStatic[9] = False
							e\room\ObjectStatic[10] = False
							updatedPD = True
						EndIf
						p2\Injuries = p2\Injuries+FPSfactor*0.00005
						If p2\PocketDimensionState = 1 Then ;in the second room
							
							;the trenches
							If EntityY(p2\Pivot)>6.0 Then
								ShouldPlay = 15
								...
								If EntityX(e\room\Objects[20],True)<EntityX(e\room\Objects[8],True)-4000*RoomScale Then
									...
									PositionEntity e\room\Objects[20], EntityX(p2\Pivot,True)+4000*RoomScale, 12.0, EntityZ(p2\Pivot,True)
								EndIf
								
								;MoveEntity(p2\Pivot, 0, Min((12.0 - EntityY(p2\Pivot)),0.0)*FPSfactor, 0)
								
								x = -FPSfactor*RoomScale*4.0
								y = (17.0-Abs(EntityX(p2\Pivot)-EntityX(e\room\Objects[20]))*0.5)-EntityY(e\room\Objects[20])
								z = EntityZ(p2\Pivot,True)-EntityZ(e\room\Objects[20])
								TranslateEntity e\room\Objects[20], x, y, z,True
								RotateEntity e\room\Objects[20], -90-(EntityX(p2\Pivot)-EntityX(e\room\Objects[20]))*1.5, -90.0, 0.0, True
								...
								For i = 0 To 2
									...
									If Distance(EntityX(p2\Pivot), EntityZ(p2\Pivot), x, z) < 200*RoomScale Then safe = True : Exit
								Next
								
								dist = EntityDistance(p2\Pivot, e\room\Objects[20])
								...
								ElseIf dist < 8.0
									...
									If dist<7.0 Then 
										...
										user_camera_pitch=user_camera_pitch-90
										RotateEntity(p2\Pivot, EntityPitch(p2\Pivot), CurveAngle(EntityYaw(pvt), EntityYaw(p2\Pivot), 10), 0)
										FreeEntity pvt
									EndIf
								EndIf
								...
								If EntityY(p2\Pivot)<8.5 Then
									...
									BlurTimer = 1500
									p2\PocketDimensionState=1
									BlinkTimer = -10
									
									PositionEntity(p2\Pivot, EntityX(e\room\Objects[8],True)-400*RoomScale, -304*RoomScale, EntityZ(e\room\Objects[8],True))
									ResetEntity p2\Pivot
							...
							Else
								...
								For i = 9 To 10
									dist = Distance(EntityX(p2\pivot), EntityZ(p2\pivot),EntityX(e\room\Objects[i],True),EntityZ(e\room\Objects[i],True))
									If dist<6.0 Then 
										If dist<100.0*RoomScale Then
											pvt=CreatePivot()
											PositionEntity pvt, EntityX(e\room\Objects[i],True),EntityY(p2\pivot),EntityZ(e\room\Objects[i],True)
											
											PointEntity pvt, p2\pivot
											RotateEntity pvt, 0, Int(EntityYaw(pvt)/90)*90,0,True
											MoveEntity pvt, 0,0,100*RoomScale
											PositionEntity p2\pivot, EntityX(pvt),EntityY(p2\pivot),EntityZ(pvt)
											...
											If Not p2\isdead Then 
												GivePlayerHealth(p2\ID, -1000, "was killed in pocket dimension")
												PositionEntity p2\Pivot, 0, 1000, 0
												ResetEntity p2\pivot
												SetPlayerPositionEx(p2\ID, p2\PlayerRoomID, EntityX(p2\Pivot), EntityY(p2\Pivot), EntityZ(p2\Pivot))
											EndIf
										EndIf
								...
								If EntityDistance(pvt, p2\Pivot)<5.0 Then 
								...
								PointEntity e\room\Objects[17], p2\Pivot
								TurnEntity e\room\Objects[17], 0, 180, 0
								
								temp = EntityDistance(p2\Pivot, e\room\Objects[17])
								If temp < 2000*RoomScale Then
									...
									user_camera_pitch=user_camera_pitch-90
									RotateEntity(p2\Pivot, EntityPitch(p2\Pivot), CurveAngle(EntityYaw(pvt), EntityYaw(p2\Pivot), Min(Max(15000.0 / (-Sanity), 15.0), 500.0)), 0)
									FreeEntity pvt
									
									;teleport the player to the trenches
									If p2\CrouchState Then
										BlinkTimer = -10
										PositionEntity p2\Pivot, EntityX(e\room\Objects[8],True)-1344*RoomScale,2944*RoomScale,EntityZ(e\room\Objects[8],True)-1184*RoomScale
										ResetEntity p2\Pivot
										...
										e\EventStr = Float(0)
									EndIf
								ElseIf EntityY(p2\Pivot)<-180*RoomScale ;the "exit room"
									temp = Distance(EntityX(p2\Pivot),EntityZ(p2\Pivot),EntityX(e\room\Objects[8],True)+1024*RoomScale,EntityZ(e\room\Objects[8],True))
									If temp<640*RoomScale
										BlurTimer = (640*RoomScale-temp)*3000
										
										e\SoundCHN2 = LoopSound2(DecaySFX(Rand(1, 3)), e\SoundCHN2, Camera, p2\Pivot, 2.0, (640*RoomScale-temp)*Abs(CurrSpeed)*100)
										...
										If temp < 130*RoomScale Then
											...
											For r.Rooms = Each Rooms
												If r\RoomTemplate\Name = "room2shaft" Then
													...
													TeleportEntity(p2\Pivot,EntityX(r\Objects[0],True),0.4,EntityZ(r\Objects[0],True),0.3,True)
													...
													SetPlayerPositionEx(p2\ID, r\ID, EntityX(p2\Pivot), EntityY(p2\Pivot), EntityZ(p2\Pivot))
							...
							If EntityY(p2\Pivot) < -1600*RoomScale Then
								If EntityDistance(p2\Pivot, e\room\Objects[8]) > 4750*RoomScale Then
									CameraFogColor Camera, 0,0,0
									CameraClsColor Camera, 0,0,0
									...
									BlurTimer = 500
									BlurTimer = 1500
									PositionEntity(p2\Pivot, EntityX(e\room\obj,True), 0.4, EntityX(e\room\obj,True))
									For r.Rooms = Each Rooms
										If r\RoomTemplate\Name = "room106" Then
											e\EventState = 0
											p2\PocketDimensionState = 0
											
											TeleportEntity(p2\Pivot,EntityX(r\Objects[10],True),0.4,EntityZ(r\Objects[10],True),0.3,True)
						...
						ElseIf p2\PocketDimensionState = 0
							dist# = EntityDistance(p2\Pivot, e\room\obj)	
							...
							If dist > 1700*RoomScale Then
								...
								Select Rand(25)
									Case 1,2,3,4
										...
										pvt = CreatePivot()
										PositionEntity(pvt, EntityX(p2\Pivot), EntityY(p2\Pivot), EntityZ(p2\Pivot))
									...
									Case 5,6,7,8,9,10 
										...
										PositionEntity(p2\Pivot, EntityX(e\room\Objects[8],True), 0.5, EntityZ(e\room\Objects[8],True))
										ResetEntity p2\Pivot
									Case 11,12 ;middle of the large starting room
										BlurTimer = 500
										PositionEntity p2\Pivot,EntityX(e\room\obj), 0.5, EntityZ(e\room\obj)
									...
									Case 16,17,18,19
										...
										For r.Rooms = Each Rooms
											If r\RoomTemplate\Name = "tunnel" Then
												...
												;econdaryLightOn = PrevSecondaryLightOn
												;PrevSecondaryLightOn = 0.0
												TeleportEntity(p2\Pivot,EntityX(r\obj,True),0.4,EntityZ(r\obj,True),0.3,True)
												SetPlayerPositionEx(p2\ID, r\ID, EntityX(p2\Pivot), EntityY(p2\Pivot), EntityZ(p2\Pivot))
									...
									Case 20,21,22 ;the tower room
										...
										PositionEntity(p2\Pivot, EntityX(e\room\Objects[12],True), 0.6, EntityZ(e\room\Objects[12],True))
										ResetEntity p2\Pivot

										p2\PocketDimensionState = 15
									Case 23,24,25
										BlurTimer = 1500
										p2\PocketDimensionState=1
										e\EventState3=1
										...
										PositionEntity(p2\Pivot, EntityX(e\room\Objects[8],True), 2288*RoomScale, EntityZ(e\room\Objects[8],True))
										ResetEntity p2\Pivot
								End Select
						...
						Else ;pillar room
							...
							If ParticleAmount > 0
								If Rand(800)=1 Then 
									...
									p.Particles = CreateParticle(EntityX(p2\Pivot)+Cos(angle)*7.5, 0.0, EntityZ(p2\Pivot)+Sin(angle)*7.5, 3, 4.0, 0.0, 2500)
									EntityBlend(p\obj, 2)
									;EntityFX(p\obj, 1)
									p\speed = 0.01
									p\SizeChange = 0
							...
							If 0 Then ; 106 eevetnt
								If p2\PocketDimensionState > 12 Then 
									Curr106\Idle = True
									PositionEntity(Curr106\Collider, EntityX(e\room\Objects[p2\PocketDimensionState],True),0.27, EntityZ(e\room\Objects[p2\PocketDimensionState],True))
									...
									If Rand(750)=1 And p2\PocketDimensionState > 12 Then
										BlinkTimer = -10
										p2\PocketDimensionState = p2\PocketDimensionState-1
										PlaySound_Strict HorrorSFX(8)
									EndIf
									
									If p2\PocketDimensionState = 12 Then
										CameraShake = 1.0
										PositionEntity(Curr106\Collider, EntityX(e\room\Objects[p2\PocketDimensionState],True),-1.0, EntityZ(e\room\Objects[p2\PocketDimensionState],True))
										Curr106\State = -10
										ResetEntity Curr106\Collider
									EndIf
							...
							If EntityY(p2\Pivot) < -1600*RoomScale Then
								...
								If Distance(EntityX(e\room\Objects[16],True),EntityZ(e\room\Objects[16],True),EntityX(p2\Pivot),EntityZ(p2\Pivot))<144*RoomScale Then
			...
			Case e_room2ccont
				...
				If PlayerInRoom(e) Then
					...
					If EntityDistance(e\room\Objects[3], e\NearObject) < 1.0 Then
						If e\EventState = 0
			...
			Case e_dimension1499
				...
				If Server\FullSync Then
					For p2.players = Each players
						If p2\PlayerRoomID<>e\room\ID
							If EntityY(p2\Pivot)>EntityY(e\room\obj)-0.5
								mp_SetPlayerRoomID(p2, e\room)
							EndIf
						EndIf
					Next
				EndIf
			...
			Case e_room2shaft
                ;[Block]
				If PlayerInRoom(e) Then
					If Player[e\NearObjectQueue]\PlayerRoomID = e\room\id Then
						If (Not Player[e\NearObjectQueue]\godmode) And (Not Player[e\NearObjectQueue]\IsDead) Then
							If EntityY(GetObject(e))<-512*RoomScale Then
								Player[e\NearObjectQueue]\DeadTicks = Player[e\NearObjectQueue]\DeadTicks + 1
								If Player[e\NearObjectQueue]\DeadTicks >= 4 Then GivePlayerHealth(e\NearObjectQueue, -10000, "")
							Else
								Player[e\NearObjectQueue]\DeadTicks = 0
							EndIf
						EndIf
					EndIf
				EndIf
			...
			Case e_room2sl
				...
				If PlayerInRoom(e) Then
					...
					If Abs(EntityY(e\room\RoomDoors[0]\frameobj)-EntityY(e\NearObject))>1.0 Then
						For i = 0 To 14
				...
				;GiveAchievement(Achv914)
					...
					If e\EventState > 0 Then
						...
						If e\EventState > (12 * 70) Then
							...
							For p2.players = Each players
								If p2\playerroomid = e\room\ID Then
									If Distance(EntityX(p2\Pivot), EntityZ(p2\Pivot), EntityX(e\room\Objects[2], True), EntityZ(e\room\Objects[2], True)) < (170.0 * RoomScale) Then
										If GetScripts() Then
											public_inqueue(public_OnItemRefine)
											public_addparam(0, setting, SE_STRING)
											public_addparam(0, EntityX(e\room\Objects[3], True), SE_FLOAT)
											public_addparam(0, EntityY(e\room\Objects[3], True), SE_FLOAT)
											public_addparam(0, EntityZ(e\room\Objects[3], True), SE_FLOAT)
											public_addparam(0, 0)
											public_addparam(0, p2\ID)
											callback
										EndIf
										
										If Not SE_RETURN_VALUE\StaticIndex Then SetPlayerPositionEx(p2\ID, p2\PlayerRoomID, EntityX(e\room\Objects[3], True), EntityY(e\room\Objects[3], True) + 1.0, EntityZ(e\room\Objects[3], True))
										Select setting
											Case "fine", "very fine"
												p2\AC_ShouldSpeed = 6.0
										End Select
									EndIf
								EndIf
							Next
			...
			Case e_gatea
				...
				;		e\room\RoomDoors[1]\liftstate = UpdateElevators(e\room\RoomDoors[1]\liftstate, e\room\RoomDoors[1], e2\room\RoomDoors[0], e2\room\Objects[1],e2\room\Objects[0])
				;		Exit
				;	EndIf
				;Next
			Case e_room2tesla
				...
				If temp And EntityY(e\NearObject, True) > EntityY(e\room\obj,True) And EntityY(e\NearObject, True) < 4.0 Then
			...
			Case e_106sinkhole
				;[Block]
				If e\EventState=0 Or e\de = Null Then
				...
				ElseIf PlayerInRoom(e) Then
					...
					If Server\FullSync Then
						For p2.players = Each players
							If p2\playerroomid = e\room\ID Then
								dist = Distance(EntityX(p2\Pivot),EntityZ(p2\Pivot),EntityX(e\room\obj),EntityZ(e\room\obj))
								If dist < 2.0 Then
									CurrStepSFX=1
									p2\CurrSpeed = CurveValue(0.0, p2\CurrSpeed, Max(dist*50,1.0))	
									CrouchState = (2.0-dist)/2.0
									If Not mp_IsASCP(p2\BreachType) Then
										If dist<0.5 Then
											
											p2\CurrSpeed = CurveValue(0.0, p2\CurrSpeed, Max(dist*50,1.0))
											
											;x = CurveValue(EntityX(e\room\obj),EntityX(p2\Pivot),10.0)
											;y = CurveValue(EntityY(e\room\obj)-p2\sinkholestate,EntityY(p2\Pivot),25.0)
											;z = CurveValue(EntityZ(e\room\obj),EntityZ(p2\Pivot),10.0)
											;PositionEntity p2\Pivot, x, y, z, True
											;SetPlayerPositionEx(p2\ID, p2\PlayerRoomID, x, y, z)
											;p2\dropspeed = 0
											;
											;ResetEntity p2\Pivot
											
											p2\sinkholestate=Min(p2\sinkholestate+FPSfactor/200.0,2.0)
											
											LightBlink = Min(p2\sinkholestate*5,10.0)
											
											If p2\sinkholestate = 2.0 Then 
												MoveToPocketDimension(p2\ID)
												p2\sinkholestate = 0
											EndIf
										EndIf
									EndIf
			...
			Case e_room3storage
				...
				If PlayerInRoom(e) Then
					...
					If EntityY(e\NearObject)<-4600*RoomScale Then
						...
						If Player[e\NearObjectQueue]\PlayerRoomID = e\room\ID Then
							If (Not Player[e\NearObjectQueue]\Godmode) And (Not Player[e\NearObjectQueue]\isdead) Then
								If EntityY(Player[e\NearObjectQueue]\Pivot)<-6400*RoomScale Then
									Player[e\NearObjectQueue]\DeadTicks = Player[e\NearObjectQueue]\DeadTicks + 1
									If Player[e\NearObjectQueue]\DeadTicks >= 4 Then GivePlayerHealth(e\NearObjectQueue, -10000, "")
								Else
									Player[e\NearObjectQueue]\DeadTicks = 0
	...
	CurrentPlayerInEvent = 0
End Function
Global CurrentPlayerInEvent

Function UpdateEvents()
	;CatchErrors("Uncaught (UpdateEvents)")
	If Server\Breach Then Return UpdateBreachEvents()
	Local dist#, i%, temp%, pvt%, strtemp$, j%, k%
		...
		Select e\EventConst
			...
			Case e_alarm ;the alarm in the starting room
				...
				Else
					...
					;DebugLog "NearObject: "+e\nearObjectQueue
					CurrTrigger = CheckTriggers(e)
					;DebugLog "CurrTrigger: "+CurrTrigger
					Select CurrTrigger
						Case "173scene_timer"
							e\EventState = e\EventState+FPSfactor
					...
					If e\EventState3>0 Then
						If e\room\NPC[3] = Null Then
							For r.Rooms = Each Rooms
								If r\RoomTemplate\Name = "start" Then
									If Server\FullSync Then
										For p2.players = Each players
											PositionEntity (p2\Pivot, EntityX(r\obj)+3584*RoomScale, 704*RoomScale, EntityZ(r\obj)+1024*RoomScale)
											ResetEntity p2\Pivot
											mp_SetPlayerRoomID(p2, e\room)
											mp_UpdatePlayerPosition(p2, True)
										Next
						...
						If e\EventState3 < 170 Then
							If e\EventState3 = 1.0 Then
								...
								MusicCHN = StreamSound_Strict("SFX\Music\"+0+".ogg",CurrMusicVolume,Mode)
								...
								If e\EventState3 < 15 Then
									...
									Else
										...
										If Server\FullSync Then
											For p2.players = Each players
												If p2\ID <> 0 Then
													SetPlayerPosition(p2\ID, e\room\RoomTemplate\Name, EntityX(Collider), EntityY(Collider), EntityZ(Collider))
												EndIf
											Next
										EndIf
										PositionEntity Collider, EntityX(Collider), 0, EntityZ(Collider)
										ResetEntity Collider
										HideEntity Collider
									EndIf
									PositionEntity Collider, EntityX(Collider), 0.302, EntityZ(Collider)
							...
							Else
								...
								Else ;inside the cell
									...
									ElseIf e\EventState3>630
										PositionEntity e\NearObject, EntityX(e\NearObject), EntityY(e\NearObject), Min(EntityZ(e\NearObject), EntityZ(e\room\obj,True)+490*RoomScale)
						...
						ElseIf e\EventState3 < 800
							...
							If e\room\NPC[5]\State <> 11
								If EntityDistance(e\room\NPC[3]\Collider,e\room\NPC[5]\Collider)>5.0 And EntityDistance(e\room\NPC[4]\Collider,e\room\NPC[5]\Collider)
									If EntityDistance(e\room\NPC[5]\Collider,e\NearObject)<3.5
						...
						ElseIf e\EventState3 < 900
							...
							If e\room\NPC[3]\State <> 11 Then
								...
								Else
									...
									e\room\NPC[4]\EnemyX = EntityX(e\NearObject)
									e\room\NPC[4]\EnemyY = EntityY(e\NearObject)
									e\room\NPC[4]\EnemyZ = EntityZ(e\NearObject)
								EndIf
							...
							If Distance(EntityX(e\room\NPC[3]\Collider),EntityZ(e\room\NPC[3]\Collider),EntityX(e\room\RoomDoors[2]\frameobj,True),EntityZ(e\room\RoomDoors[2]\frameobj,True)) < 4.5 Then
								...
								For i = 1 To Server\Maxplayers
									If Player[i] <> Null Then
										...
										If dist > 5.0 Then acceptTo = False : Exit
									EndIf
								Next
							Else
								acceptTo = False
						...
						ElseIf e\EventState3 <= 905
							...
							ElseIf e\room\NPC[3]\Frame >= 358.0 Then
								;if udp_network\stream <> 0 Then
									For i = 1 To Server\Maxplayers
										If Player[i] <> Null Then e\NearObjectQueue = i : e\NearObject = Player[i]\Pivot : Exit
									Next
								;EndIf
								e\dist = CalculateDist(e\NearObject, e\room)
								PointEntity e\room\NPC[3]\Collider,e\NearObject
								...
								Else
									...
									If (EntityDistance(e\NearObject, e\room\NPC[3]\Collider)<1.5) Then
										If EntityInView(e\room\NPC[3]\obj, GetCamera(e\NearObjectQueue)) Then
											DrawHandIcon = True
											
											If Player[e\NearObjectQueue]\MouseUpdate > MilliSecs() Then
												it.items = CreateItem("Document SCP-173", "paper", 0.0, 0.0, 0.0)
												EntityType it\collider,HIT_ITEM
												it\picker = e\NearObjectQueue
					...
					Else
							...
							ElseIf e\EventState > 230
								...
								If EntityX(e\NearObject) < (EntityX(e\room\obj)) + 408.0 * RoomScale Then
									...
									If e\EventState > 850 Then
										;For p2.players = Each players
										;	SetPlayerPosition(p2\ID, e\room\roomtemplate\name, Min(EntityX(p2\Pivot), EntityX(e\room\obj) + 352.0 * RoomScale), EntityY(p2\Pivot), EntityZ(p2\Pivot))
										;next
									End If
						...
						ElseIf e\EventState < 14000 ; player is inside the room
							...
							If e\EventState < 10300 Then
								;For p2.players = Each players
								;	SetPlayerPosition(p2\ID, e\room\roomtemplate\name, Max(EntityX(p2\Pivot), EntityX(e\room\obj) + 352.0 * RoomScale), EntityY(p2\Pivot), EntityZ(p2\Pivot))
								;next
							End If
												...
												If Server\FullSync Then
													For p2.players = Each players
														If p2\playerroomid = e\room\ID Then
															...
															PositionEntity(p2\pivot, x  + (EntityX(p2\pivot) - EntityX(e\room\obj)), y+EntityY(p2\pivot)+0.7, z + (EntityZ(p2\pivot) - EntityZ(e\room\obj)))
															p2\DropSpeed = 0
															ResetEntity(p2\pivot)
			...
			Case e_gateaentrance
				...
				If PlayerInRoom(e) Then 
					...
					Else
						...
						If Curr096 <> Null Then
							If Curr096\State = 0 Or Curr096\State = 5 Then
								e\room\RoomDoors[0]\liftstate = UpdateElevators(e\room\RoomDoors[0]\liftstate, e\room\RoomDoors[0], GateAEvent\room\RoomDoors[1], e\room\Objects[0], e\room\Objects[1],True)
							Else
						...
						If EntityDistance(Collider, e\room\Objects[1])<4.0 Then
							GateAEvent\room\RoomDoors[1]\locked = True
			...
			Case e_lockroom173
				...
				If e\dist < 6.0  And e\dist > 0 Then
					...
					Else
						...
						If (Not EntityInView(Curr173\Collider, GetCamera(e\NearObjectQueue))) Or EntityDistance(Curr173\Collider, e\NearObject)>15.0 Then 
			...
			Case e_pocketdimension
				...
				For p2.players = Each players
					If p2\playerroomid = e\room\ID Then
						...
						If p2\PocketDimensionState = 0 Then 
						...
						If EntityDistance(p2\Pivot, Curr106\Collider) < 0.3 Then ;106 attacks if close enough to player
							Curr106\Idle = False
							Curr106\State = -10
						EndIf
					...
					Else
						...
						If BlinkTimer =-10 Then
							If e\EventState > 70*7.5 And e\EventState-FPSfactor =< 70*7.5 Then
								...
								MoveEntity it\collider,0.0,0.3,0.0
								For i = 0 To 1
			...
			Case e_room2doors173
				...
				If PlayerInRoom(e) Then
					If e\EventState = 0 And Curr173\Idle = 0 Then
						...
						If (Not EntityInView(Curr173\obj, GetCamera(e\NearObjectQueue))) Then
							e\EventState = 1
				...
				If PlayerInRoom(e) Then
					...
					If e\EventState > 700 Then
						If EntityDistance(e\room\RoomDoors[0]\obj, e\NearObject)>0.5 Then
							...
							If EntityInView(e\room\RoomDoors[0]\obj, GetCamera(e\NearObjectQueue))=False Then
			...
			Case e_room3pitduck
				...
				If PlayerInRoom(e) Then
					...
					Else
						...
						If EntityInView(e\room\Objects[2],GetCamera(e\NearObjectQueue))=False Then
							e\EventState = e\EventState + FPSfactor
...
;						EndIf
						...
						Else
							If Curr096\State = 4 Then ;shoot at 096 when it starts attacking
								...
								PointEntity e\room\NPC[0]\obj, Curr096\Collider
								RotateEntity (e\room\NPC[0]\Collider, 0, CurveAngle(EntityYaw(e\room\NPC[0]\obj),EntityYaw(e\room\NPC[0]\Collider),30),0)
								If PlayerInRoom(e) Then LightBlink = (e\room\NPC[0]\Reload)+Rnd(0.5,2.0)
								Curr096\Target = e\room\NPC[0]
							Else
								If e\EventState>70*22 Then Curr096\State = 4
								If e\room\NPC[0]\State=13 Then
						...
						For do.doors = Each Doors
							If Abs(EntityX(do\obj,True)-EntityX(e\NearObject))<2.0 Then
								If Abs(EntityZ(do\obj,True)-EntityZ(e\NearObject))<2.0 Then
									...
									If (Not EntityInView(do\obj,GetCamera(e\NearObjectQueue))) Then
										If do\open Then
											DebugLog "%@@= \ {2E6C2=FD gi`h]c"
											do\open = False
											do\openstate = 0
				...
				If PlayerInRoom(e) Then
					...
					If EntityY(e\NearObject)<-4600*RoomScale Then
						...
						Else
							...
							If Player[e\NearObjectQueue]\PlayerRoomID = e\room\id Then
								If (Not Player[e\NearObjectQueue]\godmode) And (Not Player[e\NearObjectQueue]\IsDead) Then
									If EntityY(GetObject(e))<-6400*RoomScale Then
										Player[e\NearObjectQueue]\DeadTicks = Player[e\NearObjectQueue]\DeadTicks + 1
										If Player[e\NearObjectQueue]\DeadTicks >= 4 Then GivePlayerHealth(e\NearObjectQueue, -10000, "")
									Else
										Player[e\NearObjectQueue]\DeadTicks = 0
					...
					ElseIf e\EventState > 0
						...
						If e\EventState=1 Then
							If EntityDistance(e\NearObject, e\room\Objects[3])<1.2
								...
								If EntityInView(e\room\NPC[0]\obj, GetCamera(e\NearObjectQueue)) Then
					...
					Else
						...
						Select e\EventState
							...
							Case 66
								...
								EndIf
							Case 67
								If (Rand(150)=1) And (Distance(EntityX(e\NearObject), EntityZ(e\NearObject), EntityX(e\room\Objects[0],True), EntityZ(e\room\Objects[0],True))<2.0) Then
									DeathMSG = "The SCP-205 cycle seems to have resumed its normal course after the anomalies observed during "
									DeathMSG = DeathMSG + "[REDACTED]. The body of subject D-9341 was discovered inside the chamber. "
									DeathMSG = DeathMSG + "The subject exhibits signs of blunt force trauma typical for personnel who have "
									DeathMSG = DeathMSG + "entered the chamber when the lights are off."
									
									GivePlayerHealth(e\NearObjectQueue, -15, "was killed by SCP-205")
									PlaySound_Strict DamageSFX(Rand(2,3))
									;mp_writesound DamageSFX(Rand(2,3))
									CameraShake = 0.5
			...
			Case e_room860
				...
				If fr <> Null Then
					If Server\FullSync Then
						For p2.Players = Each players
							If p2\playerroomid = e\room\ID Then
								If (Abs(Distance(EntityX(e\room\Objects[3],True),EntityZ(e\room\Objects[3],True),EntityX(p2\Pivot,True),EntityZ(p2\Pivot,True)))<1.0) Then
									If p2\MouseUpdate > MilliSecs() Then
										PlaySound_Strict(LoadTempSound("SFX\Door\WoodenDoorOpen.ogg"))
										;Sound "SFX\Door\WoodenDoorOpen.ogg"
										ShowEntity fr.Forest\Forest_Pivot
				...
				If e\EventState = 0 Then
					If e\dist < 5.0 And e\dist > 0 Then
						...
						Else
							...
							If Curr106\State <= -10 And EntityDistance(Curr106\Collider,e\NearObject)>5 And (Not EntityInView(Curr106\obj,GetCamera(e\NearObjectQueue))) Then
								e\EventState = 1
								e\EventState2 = 1
							EndIf
			...
			Case e_toiletguard
				...
				Else
					...
					ElseIf e\dist<4.0 And Player[e\NearObjectQueue]\PlayerSoundVolume > 1.0
						If e\EventState2=0
							;Y=0.01
						...
						If dist<2.0 Then 
							...
							If e\EventState2=0 Then
								...
								If EntityDistance(Curr173\Collider,e\room\Objects[4])<3.0
									...
									If (Player[e\NearObjectQueue]\BlinkTimer<-10 Or (Not EntityInView(Curr173\obj,GetCamera(e\NearObjectQueue)))) And Curr173\Idle = 0 Then
							...
							If dist < 2.0 Then
								...
								If dist<0.5 Then
									If e\EventState2 = 0 Then
										...
										;Sound "SFX\Room\SinkholeFall.ogg"
									EndIf
									
									p2\CurrSpeed = CurveValue(0.0, p2\CurrSpeed, Max(dist*50,1.0))
									
									;x = CurveValue(EntityX(e\room\obj),EntityX(p2\Pivot),10.0)
									;y = CurveValue(EntityY(e\room\obj)-e\EventState2,EntityY(p2\Pivot),25.0)
									;z = CurveValue(EntityZ(e\room\obj),EntityZ(p2\Pivot),10.0)
									;PositionEntity p2\Pivot, x, y, z, True
									;SetPlayerPositionEx(p2\ID, p2\PlayerRoomID, x, y, z)
									...
									If e\EventState2 = 2.0 Then MoveToPocketDimension(p2\ID)
				...
				If e\EventState = 1
					If e\EventState2 < 0
						If e\EventState2 = -(70*5)
							For sc.SecurityCams = Each SecurityCams
								If sc\room = e\room
									If EntityDistance(sc\ScrObj,Player[e\NearObjectQueue]\Pivot)<5.0
										If EntityVisible(sc\ScrObj,Player[e\NearObjectQueue]\Pivot)
								...
								If Abs(EntityY(e\room\RoomDoors[0]\frameobj)-EntityY(e\room\NPC[0]\Collider))>1.0
									If Abs(EntityY(e\room\RoomDoors[0]\frameobj)-EntityY(e\NearObject))<1.0
										If e\room\RoomDoors[0]\open
											...
											;if Server\MainPlayer = False Then
											;	udp_WriteByte(M_DOOR)
											;	udp_WriteByte(Server\MyID)
...
Function UpdateDimension1499()
	...
	If QuickLoadPercent = 100 Or QuickLoadPercent = -1
		...
		For p2.players = Each players
			If p2\playerroomid = e\room\id Then
				ShowEntity e\room\obj
				UpdateChunks(e\room,15, True, p2\Pivot, p2)
				ShouldPlay = 18
				If EntityY(p2\Pivot)<800.0
					PositionEntity p2\Pivot,EntityX(p2\Pivot),800.5,EntityZ(p2\Pivot),True
					ResetEntity p2\Pivot
				EndIf
			EndIf
		Next
		...
		Else
			...
			If e\EventState < 2.0 And SelectedEnding = "" Then 
				...
				If EntityDistance(e\NearObject, e\room\Objects[10])<320*RoomScale Then
					...
					PlayAnnouncement("SFX\Ending\GateB\682Battle.ogg", False)
				EndIf								
			Else
				If PlayerRoom = e\room Then ShouldPlay = 6
				e\EventState=e\EventState+FPSfactor
				...
				Else
					If SelectedEnding = "" Then
						...
						Else
							...
							;e\SoundCHN = PlaySound_Strict(e\Sound)
							PlayAnnouncement("SFX\Ending\GateB\AlphaWarheadsFail.ogg", False)
