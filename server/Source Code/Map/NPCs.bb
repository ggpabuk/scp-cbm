...
Function CreateNPC.NPCs(NPCtype%, x#, y#, z#)
	...
	Select NPCtype
		...
		Case NPCtypeClerk
			...
			;[End Block]
	End Select
	if NPCtype <> NPCtypeClassDBot Then
	...
	if GetScripts() Then
		public_inqueue(public_OnCreateNPC)
		public_addparam(0, n\ID)
		callback
	EndIf
		...
		Select n\NPCtype
						...
						If n\Idle = False Then
								...
								For pl.players = Each players
									if pl\id <> 0 Then
										If (pl\BlinkTimer < - 16 Or pl\BlinkTimer > - 6) Then
											If EntityInView(n\obj,GetCamera(pl\ID)) Then move = False
										EndIf
									EndIf
								Next
							EndIf
							...
							Else 
								...
								Else ;less than 3 rooms away -> actively move towards the player
									...
									If temp Then 				
										If dist < 0.65 Then
											If (Not Player[n\NearID]\GodMode) and (Not Player[n\NearID]\IsDead) Then
												...
												if Not Instr(Player[n\NearID]\LatestPlayerSound, "Neck") Then CreateSound("SFX\SCP\173\NeckSnap"+Rand(1,3)+".ogg", EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject))
												If Rand(2) = 1 Then 
													TurnEntity(Camera, 0, Rand(80,100), 0)
												Else
													TurnEntity(Camera, 0, Rand(-100,-80), 0)
												EndIf
												GivePlayerHealth(n\NearID, -1000, "was killed by SCP-173")
											EndIf
										Else
					...
					If (Not n\Idle) And spawn106%
						If n\State <= 0 Then	;attacking	
							If EntityY(n\Collider) < EntityY(n\NearObject) - 20.0 - 0.55 And (Not ChannelPlaying(n\channelmultiplayer)) Then
								...
								if 0 Then n\channelmultiplayer = PlaySound_Strict(DecaySFX(0))
							End If
								...
								ElseIf Player[n\NearID]\PlayerRoomName <> "gatea" And (Not NoTarget) ;dist < 0.8
									...
									If KillTimer >= 0 And FallTimer >= 0 Then								
										If Ceil(n\Frame) = 110 And (Not GodMode) Then
											...
											;mp_writesound(DamageSFX(1),EntityX(Collider),EntityY(Collider),EntityZ(Collider))
											if Not Instr(Player[n\NearID]\LatestPlayerSound, "Damage") Then CreateSound("SFX\Character\D9341\Damage1.ogg", EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject))
											If Player[n\NearID]\PlayerRoomName = "pocketdimension" Then
												...
												GivePlayerHealth(n\NearID, -1000, "was killed by SCP-106")
											Else
												...
												MoveToPocketDimension(n\NearID)
											EndIf
											
											n\State = -20000
											TranslateEntity(n\Collider,0,-50.0,0,True)
										EndIf
									EndIf
								EndIf
							...
							If n\Reload = 0 Then
                                If dist > 10 And Player[n\NearID]\PlayerRoomName <> "pocketdimension" And Player[n\NearID]\PlayerRoomName <> "gatea" And n\State <-5 Then ;timer idea by Juanjpro
                                    If (Not EntityInView(n\obj,GetCamera(n\NearID)))
								...
								For i = 1 To Server\maxplayers
									if Player[i] <> Null Then
										if Not Player[i]\IsDead Then
											angle# = WrapAngle(DeltaYaw(n\Collider, Player[i]\camera)) ;-EntityYaw(n\Collider,True))
											If angle<90 Or angle>270 Then
												
												CameraProject Player[i]\camera,EntityX(n\Collider), EntityY(n\Collider)+0.25, EntityZ(n\Collider)
												If ProjectedX()>0 And ProjectedX()<Player[i]\monitor_width Then
													If ProjectedY()>0 And ProjectedY()<Player[i]\monitor_height Then
														If EntityVisible(Player[i]\Pivot, n\Collider) Then
															If (Player[i]\BlinkTimer < - 16 Or Player[i]\BlinkTimer > - 6) And (Not Player[i]\IsDead) Then
						...
						If dist < 16.0 Then 
							...
							If (Not NoTarget)
								For i = 1 To Server\Maxplayers
									if Player[i] <> Null Then
										if Not Player[i]\IsDead Then
											angle = WrapAngle(DeltaYaw(n\Collider, Player[i]\Camera))
											If angle<55 Or angle>360-55 Then
												CameraProject Player[i]\Camera,EntityX(n\Collider), EntityY(Player[i]\Pivot)+5.8*0.2-0.25, EntityZ(n\Collider)
							...
							If (dist < HideDistance*2) And (Not n\Idle) Then
								...
								If PlayerSeeAble%=True Or n\State2>0 Then ;Player is visible for 049's sight - attacking
									...
									If n\PrevState <= 1 And ChannelPlaying(n\SoundChn2)=False
										...
										;;Sound("SFX\SCP\049\Spotted"+syncRand+".ogg", EntityX(n\obj),EntityY(n\obj),EntityZ(n\obj))
									...
									If dist < 0.6 Then
										local cankill% = false
										If Player[n\NearID]\WearingHazmat>0 Then
											...
											If BlurTimer>250 And BlurTimer-FPSfactor*2.5 <= 250 And n\PrevState<>3 Then
												...
												;;Sound("SFX\SCP\049\TakeOffHazmat.ogg", EntityX(n\obj),EntityY(n\obj),EntityZ(n\obj))
												n\PrevState=3
											ElseIf BlurTimer => 500
												cankill = true
												blurtimer = 0
											EndIf
										ElseIf ReadBool(Player[n\NearID]\wearingdir, 4) Then
											BlurTimer = BlurTimer+FPSfactor*2.5
											If BlurTimer>250 And BlurTimer-FPSfactor*2.5 <= 250 And n\PrevState<>3 Then
												If n\SoundChn2 <> 0 Then StopChannel(n\SoundChn2)
												n\SoundChn2 = PlaySound_Strict(LoadTempSound("SFX\SCP\049\714Equipped.ogg"))
												;;Sound("SFX\SCP\049\714Equipped.ogg", EntityX(n\obj),EntityY(n\obj),EntityZ(n\obj))
												n\PrevState=3
											ElseIf BlurTimer => 500
												cankill = true
												blurtimer = 0
										...
										if cankill then
											If (Not Player[n\NearID]\GodMode) Then
												...
												n\SoundChn2 = LoopSound2(n\Sound2,n\SoundChn2,Camera,n\obj)
												;;Sound("SFX\SCP\049\Kidnap"+syncRand+".ogg", EntityX(n\obj),EntityY(n\obj),EntityZ(n\obj))
												n\State = 1
												if Not Instr(Player[n\NearID]\LatestPlayerSound, "Horror") Then CreateSound("SFX\Horror\Horror13.ogg", EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject))
											EndIf
											cankill	= false								
										EndIf
									Else
										...
										If n\Path[n\PathLocation]<>Null Then
											...
											If n\PrevState = 0 And ChannelPlaying(n\SoundChn2)=False
												...
												If Rand(30)=1
													...
													;;Sound("SFX\SCP\049\Searching7.ogg", EntityX(n\obj), EntityY(n\obj), EntityZ(n\obj))
												Else
													syncRand = Rand(1,6)
													n\Sound2 = LoadSound_Strict("SFX\SCP\049\Searching"+syncRand+".ogg")
												;	;Sound("SFX\SCP\049\Searching"+syncRand+".ogg", EntityX(n\obj), EntityY(n\obj), EntityZ(n\obj))
												EndIf
												n\SoundChn2 = LoopSound2(n\Sound2,n\SoundChn2,Camera,n\obj)
												n\PrevState = 1
											EndIf
										...
										If n\PathTimer# > 70*(5-(2*SelectedDifficulty\aggressiveNPCs)) Then
											...
											While Int(n\State3) < 3
												...
												If n\PathStatus <> 1 Then
													...
													closestRoom.Rooms = Room[Player[n\NearID]\PlayerRoomID]
													currdist# = 0.0
													For i = 0 To 3
														If Room[Player[n\NearID]\PlayerRoomID]\Adjacent[i]<>Null Then
															currdist = EntityDistance(Room[Player[n\NearID]\PlayerRoomID]\Adjacent[i]\obj,n\Collider)
										...
										If EntityDistance(n\Collider,n\NearObject)<1.1
											If (Abs(DeltaYaw(n\Collider,n\NearObject))<=60.0) Then
												if 0 Then
													...
													;mp_writesound(DamageSFX(Rand(5,8)), EntityX(Collider), EntityY(Collider), EntityZ(Collider), 5.0)
												...
												Else
													GivePlayerHealth(n\NearID, -18, "was killed by SCP-049-2")
													if Not Instr(Player[n\NearID]\LatestPlayerSound, "Damage") Then CreateSound("SFX\Character\D9341\Damage"+Rand(6,8)+".ogg", EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject))
												EndIf
											EndIf
										EndIf
			...
			Case NPCtype372 ;------------------------------------------------------------------------------------------------------------------
				...
				If RN$ <> "pocketdimension" And RN$ <> "dimension1499" Then 
					If n\Idle Then
						HideEntity(n\obj)
						If Rand(50) = 1 And (Player[n\NearID]\BlinkTimer < -5 And Player[n\NearID]\BlinkTimer > -15) Then
							ShowEntity(n\obj)
					...
					Else
						...
						If EntityInView(n\obj, GetCamera(n\NearID)) And (Player[n\NearID]\BlinkTimer < - 16 Or Player[n\NearID]\BlinkTimer > - 6) Then
						...
						Case 2
							...
							Else
								...
								If n\Frame>=5 And n\Frame<6 Then
									If dist < 1.8 Then
										If Abs(DeltaYaw(n\Collider, n\NearObject))<20 Then 
											If Player[n\NearID]\WearingHazmat Then
												Injuries = Injuries+Rnd(0.5)
												PlaySound_Strict(LoadTempSound("SFX\General\BodyFall.ogg"))
												if Not Instr(Player[n\NearID]\LatestPlayerSound, "BodyFall") Then CreateSound("SFX\General\BodyFall.ogg", EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject))
											Else
												BlurTimer = 100
												Injuries = Injuries+Rnd(1.0,1.5)
												PlaySound_Strict DamageSFX(Rand(2,3))
												if Not Instr(Player[n\NearID]\LatestPlayerSound, "Damage") Then CreateSound("SFX\Character\D9341\Damage"+Rand(3,4)+".ogg", EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject))
												;mp_writesound(DamageSFX(Rand(2,3)), EntityX(Collider), EntityY(Collider), EntityZ(Collider), 5.0)
								...
								If (n\Frame=>18.0 And n\Frame<68.0) Then
									...
									If temp Then
										If Distance(EntityX(n\NearObject), EntityZ(n\NearObject), EntityX(n\Collider), EntityZ(n\Collider))<1.5 Then
											if 0 Then
												PlaySound_Strict n\Sound2
												;Sound("SFX\General\Slash1.ogg", EntityX(Collider), EntityY(Collider), EntityZ(Collider), 5.0)
												Injuries = Injuries + Rnd(1.5, 2.5)-WearingVest*0.5
												BlurTimer = 500
											Else
												GivePlayerHealth(n\NearID, -50, "was killed by SCP-939")
												if Not Instr(Player[n\NearID]\LatestPlayerSound, "Slash") Then CreateSound("SFX\General\Slash1.ogg", EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject))
											EndIf
										Else
											n\Frame	 = 449
					...
					If n\State < 3 And (Not NoTarget) And (Not n\IgnorePlayer) Then
						...
						If Player[n\NearID]\PlayerSoundVolume*1.2>dist Or dist < 1.5 Then
							If n\State3 = 0 Then
								...
								if 0 Then PlaySound_Strict(LoadTempSound("SFX\SCP\939\attack.ogg"))
								n\State3 = 1
							EndIf
								...
								If n\State2 = 0 Then
									...
									Select Rand(1,6)
										...
										Case 3
											...
											Next
										Case 4
											If PlayerRoom\RoomTemplate\DisableDecals = False And 0 Then
												CameraShake = 5.0
												de.Decals = CreateDecal(1, EntityX(n\Collider), 0.01, EntityZ(n\Collider), 90, Rand(360), 0)
												de\Size = 0.3 : UpdateDecals
												PlaySound_Strict(LoadTempSound("SFX\General\BodyFall.ogg"))
												;Sound("SFX\General\BodyFall.ogg", EntityX(Collider), EntityY(Collider), EntityZ(Collider), 5.0)
...
;								EndIf
							...
							If dist<1.0 Then
								...
								If n\Frame>470.0 And prevFrame<=470.0 Or n\Frame>500.0 And prevFrame<=500.0 Or n\Frame>527.0 And prevFrame<=527.0
									if 0 Then
										...
										;Sound("SFX\General\Slash"+Rand(1,2)+".ogg", EntityX(n\Collider), EntityY(n\Collider), EntityZ(n\Collider))
									
										Injuries = Injuries + Rnd(0.5,1.0)
									Else
										GivePlayerHealth(n\NearID, -15, "was killed by SCP-966")
										if Not Instr(Player[n\NearID]\LatestPlayerSound, "Slash") Then CreateSound("SFX\General\Slash"+Rand(1,2)+".ogg", EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject))
									EndIf
								EndIf	
							EndIf
							...
							If n\State2 = 1
								...
								If prevFrame < 89 And n\Frame=>89
									...
									ElseIf 0
										...
										PlaySound2(LoadTempSound("SFX\General\Slash"+Rand(1,2)+".ogg"), Camera, n\Collider)
										;Sound("SFX\General\Slash"+Rand(1,2)+".ogg")
										
										If Injuries > 10.0
											Kill("was killed by SCP-1499")
										...
										EndIf
									Else
										GivePlayerHealth(n\NearID, -20, "was killed by SCP-1499")
										if Not Instr(Player[n\NearID]\LatestPlayerSound, "Slash") Then CreateSound("SFX\General\Slash"+Rand(1,2)+".ogg", EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject))
									EndIf
								ElseIf n\Frame => 99
									n\State2 = 0.0
							...
							Else
								...
								If prevFrame < 189 And n\Frame=>189
									...
									Else If 0
										Injuries = Injuries + Rnd(0.75,1.5)
										PlaySound2(LoadTempSound("SFX\General\Slash"+Rand(1,2)+".ogg"), Camera, n\Collider)
										;mp_writesound("SFX\General\Slash"+Rand(1,2)+".ogg", EntityX(Collider), EntityY(Collider), EntityZ(Collider))
					...
					Select n\State
						...
						Case 3 ;Attacking
							...
							If (n\Frame => 146 And prevFrame < 146)
								If EntityDistance(n\Collider,n\NearObject)<1.1
									If (Abs(DeltaYaw(n\Collider,n\NearObject))<=60.0) And 0 Then
										PlaySound_Strict DamageSFX(Rand(5,8))
										;mp_writesound(DamageSFX(Rand(5,8)), EntityX(Collider), EntityY(Collider), EntityZ(Collider), 5.0)
										Injuries = Injuries+Rnd(0.4,1.0)
		...
		if n\NPCtype = NPCtypeZombie then DebugLog EntityY(n\Collider)
	Next
...
Function FindNearestID(n.NPCs)
	...
	For p.players = Each players
		if (Not p\IsDead) And p\ID <> 0 And (Not mp_IsASCP(p\BreachType)) Then
			nearid% = p\ID
			dist2 = EntityDistance(n\Collider,p\Pivot)
			For ps.players = Each players
				If p <> ps And (Not ps\IsDead) And ps\ID <> 0 And (Not mp_IsASCP(ps\BreachType)) Then
...
Function Shoot(n.NPCs, x#, y#, z#, hitProb# = 1.0, particles% = True, instaKill% = False)
	...
	If instaKill Then
		If (Not GodMode) Then
			GivePlayerHealth(n\NearID, -1000, "was killed by shoot")
			if Not Instr(Player[n\NearID]\LatestPlayerSound, "BulletHit") Then CreateSound("SFX\General\BulletHit.ogg", EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject))
		EndIf
	EndIf
	...
	If Rnd(1.0) =< hitProb Then
		If (Not Player[n\NearID]\Godmode) Then
			...
			If Player[n\nearid]\wearingvest>0 Then
				If Player[n\nearid]\wearingvest = 1 Then
					Select Rand(8)
						Case 1,2,3,4,5
							BlurTimer = 500
							Stamina = 0
							ShotMessageUpdate = "A bullet penetrated your vest, making you gasp."
							GivePlayerHealth(n\NearID, -Rand(10,15), "was killed by shoot")
						Case 6
							BlurTimer = 500
							ShotMessageUpdate = "A bullet hit your left leg."
							GivePlayerHealth(n\NearID, -Rand(20,25), "was killed by shoot")
						Case 7
							BlurTimer = 500
						...
						Case 8
							...
							GivePlayerHealth(n\NearID, -Rand(40,50), "was killed by shoot")
					End Select	
				Else
					If Rand(10)=1 Then
						BlurTimer = 500
				...
				If GetPlayerInjuries(n\NearID) >= 5
					If Rand(3) = 1 Then GivePlayerHealth(n\NearID, -1000, "was killed by shoot")
				EndIf
			Else
				Select Rand(6)
					Case 1
					...
					Case 6
						BlurTimer = 500
						ShotMessageUpdate = "A bullet hit your right shoulder."
						GivePlayerHealth(n\NearID, -Rand(30,45), "was killed by shoot")
				End Select
			EndIf
			if Not Instr(Player[n\NearID]\LatestPlayerSound, "BulletHit") Then CreateSound("SFX\General\BulletHit.ogg", EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject))
		Else
			GivePlayerHealth(n\NearID, -Rand(20, 30), "was killed by shoot")
			if Not Instr(Player[n\NearID]\LatestPlayerSound, "BulletHit") Then CreateSound("SFX\General\BulletHit.ogg", EntityX(n\NearObject),EntityY(n\NearObject),EntityZ(n\NearObject))
		EndIf
	EndIf
End Function
